...
static SENSOR_SC1035_DO_I2CRD sensor_i2c_read = NULL;
static SENSOR_SC1035_DO_I2CWR sensor_i2c_write = NULL;
...
HI_U32 gu32FullLinesStd = 1000;

 static HI_U8  gu8SensorMode = 0;
 
 
 static const unsigned char sensor_i2c_addr  =	 0x60;		 /* I2C Address of SC1035 */
...
static HI_S32 sc1035_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->stAgainAccu.enAccuType = AE_ACCURACY_LINEAR;//AE_ACCURACY_DB;//AE_ACCURACY_TABLE;
    pstAeSnsDft->stAgainAccu.f32Accuracy = 0.0588;//0.0588*64;//1 / 0x40;    
    pstAeSnsDft->u32MaxAgain = 1706;//~0x40 * 106.25;//1700;//~0x40 * 24.094;
    pstAeSnsDft->u32MinAgain = 64;//0x40;//1024;
    pstAeSnsDft->u32MaxAgainTarget = 1706;//~0x40 * 106.25;//1700;//0x40 * 24.094;
    pstAeSnsDft->u32MinAgainTarget = 64;//0x40;//1024;

    pstAeSnsDft->stDgainAccu.enAccuType = AE_ACCURACY_LINEAR;
    pstAeSnsDft->stDgainAccu.f32Accuracy = 1;//0.0588;//0.03125;    
    pstAeSnsDft->u32MaxDgain = 1;//17;//32;//255;  /* (8 - 0.3125) / 0.03125 = 255 */
    pstAeSnsDft->u32MinDgain = 1;//17;//32;
    pstAeSnsDft->u32MaxDgainTarget = 1;//17;//32;//255;
    pstAeSnsDft->u32MinDgainTarget = 1;//17;//32;
...
static HI_S32 sc1035_get_sensor_max_resolution(ISP_CMOS_SENSOR_MAX_RESOLUTION *pstSensorMaxResolution)
{
...
static HI_VOID sc1035_fps_set(HI_U8 u8Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    switch(u8Fps)
    {
        case 30:
...
static HI_VOID sc1035_slow_framerate_set(HI_U16 u16FullLines,
...
static HI_VOID sc1035_init_regs_info(HI_VOID)
{
#if CMOS_SC1035_ISP_WRITE_SENSOR_ENABLE
    HI_S32 i;
    ...
    if (HI_FALSE == bInit)
    {
        ...
        g_stSnsRegsInfo.astI2cData[3].u32RegAddr = 0x3e0f;//0x3100;
...
{
	HI_U32 u32GReg = 0, u32Temp = 1, u32DGReg = 0;

	if(u32Again < (120)){//1.883*64
		if(u32Again >= (2 * 0x40)){
			u32GReg |= 0x00;
			u32Temp = 2;
		}

		if(u32Again < 0x40){	//Gain less than 1, OV5653 setting must larger or equal to 1
			u32GReg = 0x0;
		}
		else{
			u32Again /= u32Temp;
			//u32GReg |= ((u32Again - 64)/4)&0x0f;
			u32Temp = ((17*(u32Again-64))/64);
			if(u32Temp > 0x0f)
				u32Temp = 0x0f;

			//u32GReg |= ((17*(u32Again-64))/64 )&0x0f;
			u32GReg |= u32Temp & 0x0f;
	...
	else if(u32Again < (226)){//3.54*0x40
		u32GReg = 0x80;
		u32Temp = 116;//1.835*64;
		u32Temp = (17*u32Again / u32Temp -17);
		if(u32Temp > 0x0f){
			u32Temp = 0x0f;
		}
		u32GReg |= u32Temp & 0x0f;
	...
	else if(u32Again < (453)){//7.08*0x40
		u32GReg = 0x90;
		u32Temp = 226; //3.54*0x40
		u32Temp = (17*u32Again / u32Temp -17);
		if(u32Temp > 0x0f){
			u32Temp = 0x0f;
		}
	...
	else if(u32Again < (906)){//14.16*0x40
		u32GReg = 0xb0;
		u32Temp = 453; //7.08*0x40
		u32Temp = (17*u32Again / u32Temp -17);
		if(u32Temp > 0x0f){
			u32Temp = 0x0f;
		}
	...
	else if(u32Again < (1812)){//28.32*0x40
		u32GReg = 0xf0;
		u32Temp = 906; //14.16*0x40
		u32Temp = (17*u32Again / u32Temp -17);
		if(u32Temp > 0x0f){
			u32Temp = 0x0f;
		}
	...
	else if(u32Again < (3624)){//56.64*0x40
		u32DGReg = 0x1;
		u32GReg = 0xf0;
		u32Temp = 1812; //28.32*0x40
		u32Temp = (17*u32Again / u32Temp -17);
		if(u32Temp > 0x0f){
			u32Temp = 0x0f;
		}
...
//printf("GReg: 0x%x\t 0x%x\n",u32GReg,u32DGReg);
	#if CMOS_SC1035_ISP_WRITE_SENSOR_ENABLE
	    sc1035_init_regs_info();
	    g_stSnsRegsInfo.astI2cData[2].u32Data = sc1035_Again_limit(u32GReg);
	    g_stSnsRegsInfo.astI2cData[3].u32Data = (u32DGReg | 0x14);
	    HI_MPI_ISP_SnsRegsCfg(&g_stSnsRegsInfo);
	#else
	    SENSOR_I2C_WRITE(0x3e09, sc1035_Again_limit(u32GReg));
	#endif
...
void sc1035_reg_init()
{
	...
	SENSOR_I2C_WRITE(0x3211, 0x62);//
	SENSOR_I2C_WRITE(0x3213, 0x08);
	SENSOR_I2C_WRITE(0x5054, 0x82);
	SENSOR_I2C_WRITE(0x320c, 0x06);//1750
	SENSOR_I2C_WRITE(0x320d, 0xd6);
	SENSOR_I2C_WRITE(0x320e, 0x04);//1200
	SENSOR_I2C_WRITE(0x320f, 0xb0);

	//blc
	//SENSOR_I2C_WRITE(0x3900, 0x01);
	SENSOR_I2C_WRITE(0x3907, 0x02);//0x00);
	SENSOR_I2C_WRITE(0x3908, 0x00);//0xc0);

	//exp
	//SENSOR_I2C_WRITE(0x3e01, 0x31);
	//SENSOR_I2C_WRITE(0x3e02, 0xc0);

	//gain
	//SENSOR_I2C_WRITE(0x3e08, 0x01);
	//SENSOR_I2C_WRITE(0x3e09, 0x30);
	//delay_ms(200);	//DELAY= 200

	//color bar
	//SENSOR_I2C_WRITE(0x503d, 0x80);
#elif SENSORCLK_72M
	SENSOR_I2C_WRITE(0x3000, 0x01);
	SENSOR_I2C_WRITE(0x3000, 0x01);
	SENSOR_I2C_WRITE(0x3000, 0x01);
	...
	SENSOR_I2C_WRITE(0x3213, 0x08);
	SENSOR_I2C_WRITE(0x5054, 0x82);
	SENSOR_I2C_WRITE(0x320c, 0x07);//1750
	SENSOR_I2C_WRITE(0x320d, 0x80);
	SENSOR_I2C_WRITE(0x320e, 0x04);//1200
	SENSOR_I2C_WRITE(0x320f, 0xe2);
	...
	SENSOR_I2C_WRITE(0x320d, 0x08);
#endif
	SENSOR_I2C_WRITE(0x320e, 0x03);//1200
	SENSOR_I2C_WRITE(0x320f, 0xe8);
	...
	SENSOR_I2C_WRITE(0x3323, 0xf0);//1750
	SENSOR_I2C_WRITE(0x3315, 0x44);
	SENSOR_I2C_WRITE(0x3301, 0x83);//1200
...
#endif
	printf("YF1412192_SC1035 sensor 960P30fps 63MHz init success!\n\n");
...
HI_S32 sc1035_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    ...
    pstSensorExpFunc->pfn_cmos_get_sensor_max_resolution = sc1035_get_sensor_max_resolution;
...
int sc1035_sensor_register_callback(void)
{
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
    ...
    s32Ret = HI_MPI_ISP_SensorRegCallBack(SC1035_ID, &stIspRegister);
    if (s32Ret)
    {
    ...
    s32Ret = HI_MPI_AE_SensorRegCallBack(&stLib, SC1035_ID, &stAeRegister);
    if (s32Ret)
    {
    ...
    s32Ret = HI_MPI_AWB_SensorRegCallBack(&stLib, SC1035_ID, &stAwbRegister);
    if (s32Ret)
    {
        printf("sensor register callback function to ae lib failed!\n");
        return s32Ret;
...
int sc1035_sensor_unregister_callback(void)
{
    HI_S32 s32Ret;
    ALG_LIB_S stLib;

    s32Ret = HI_MPI_ISP_SensorUnRegCallBack(SC1035_ID);
    if (s32Ret)
    {
    ...
    stLib.s32Id = 0;
    strcpy(stLib.acLibName, HI_AE_LIB_NAME);
    s32Ret = HI_MPI_AE_SensorUnRegCallBack(&stLib, SC1035_ID);
    if (s32Ret)
    {
    ...
    stLib.s32Id = 0;
    strcpy(stLib.acLibName, HI_AWB_LIB_NAME);
    s32Ret = HI_MPI_AWB_SensorUnRegCallBack(&stLib, SC1035_ID);
    if (s32Ret)
    {
        printf("sensor unregister callback function to ae lib failed!\n");
        return s32Ret;
...
void SC1035_init(SENSOR_SC1035_DO_I2CRD do_i2c_read, SENSOR_SC1035_DO_I2CWR do_i2c_write)
{// init i2c buf
	...
	sc1035_sensor_register_callback();
	...
	printf("SC1035 sensor 720P30fps init success!\n");
