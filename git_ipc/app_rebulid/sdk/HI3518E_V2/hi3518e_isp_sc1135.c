...
#define SENSOR_SC1135_WIDTH 1280
#define SENSOR_SC1135_HEIGHT 960
#define SC1135_CHECK_DATA_MSB (0x00)	//0X580B
#define SC1135_CHECK_DATA_LSB (0x00)	//0X3108
#define SC1135_CHECK_DATA_HSB (0x53)	//0X3400
...
static HI_S32 sc1135_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    pstAeSnsDft->u32MaxAgain = 15400;  //62�� 
...
static HI_VOID sc1135_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID sc1135_slow_framerate_set(HI_U32 u32FullLines,
...
static HI_VOID sc1135_again_calc_table(HI_U32 *pu32AgainLin, HI_U32 *pu32AgainDb)
{
    int i;
...
static HI_VOID sc1135_dgain_calc_table(HI_U32 *pu32DgainLin, HI_U32 *pu32DgainDb)
{
    int i;
...
static HI_VOID sc1135_gains_update(HI_U32 u32Again, HI_U32 u32Dgain)
{
...
#else
    SENSOR_I2C_WRITE(0x3e09, sc1135_Again_limit((u8AgainHigh<<4) | u8AgainLow));
    SENSOR_I2C_WRITE(0x3e08, u32DGReg);
#endif
	...
	{
		SENSOR_I2C_WRITE(0x3630,0xb8);
		SENSOR_I2C_WRITE(0x3631,0x82);
	}
	else if((u8DgainVal == 0) && (u8AgainVal >= 0x10) && (u8AgainVal<=0x3e))
	{
		SENSOR_I2C_WRITE(0x3630,0x70);
...
{   
    ...
    pstDef->stSensorMaxResolution.u32MaxWidth  = SENSOR_SC1135_WIDTH;
    pstDef->stSensorMaxResolution.u32MaxHeight = SENSOR_SC1135_HEIGHT;
...
HI_U32 sc1135_get_sns_regs_info(ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
...
static HI_S32 sc1135_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
...
static void sc1135_reg_init()
{
    ...
    SENSOR_I2C_WRITE(0x3d08,0x01);//black point 2016.07.22
    ...
    SENSOR_I2C_WRITE(0x3601,0x1a); //20160422
    ...
    SENSOR_I2C_WRITE(0x5785,0x0c); //20160328
    ...
    SENSOR_I2C_WRITE(0x3335,0x06); //20160418
    SENSOR_I2C_WRITE(0x3880,0x00); 
    
    //power reduction 20160606
    SENSOR_I2C_WRITE(0x3620,0x42);
    SENSOR_I2C_WRITE(0x3610,0x03);
    SENSOR_I2C_WRITE(0x3600,0x64);
    SENSOR_I2C_WRITE(0x3636,0x0d);
    SENSOR_I2C_WRITE(0x3323,0x80);
   ...
   //  SENSOR_I2C_WRITE(0x3610,0x2b);  //20160606
	...
	SENSOR_I2C_WRITE(0x3213,0x04); //for BGGR out format 20160412
	...
	SENSOR_I2C_WRITE(0x3321,0xd8);
	SENSOR_I2C_WRITE(0x3322,0x01);
	  //  SENSOR_I2C_WRITE(0x3323,0x80); //20160606
	  //  SENSOR_I2C_WRITE(0x3600,0x54); //20160606
    bSensorInit = HI_TRUE;
    printf("======================================================================\n");
    printf("===SmartSens sc1135 sensor 960P30fps(Parallel port) init success!=====\n");
    printf("======================================================================\n");
...
HI_S32 sc1135_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
   ...
   // pstSensorExpFunc->pfn_cmos_sensor_exit = sensor_exit;
    ...
    pstSensorExpFunc->pfn_cmos_set_image_mode = sc1135_set_image_mode;
    ...
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = sc1135_get_sns_regs_info;
...
int sc1135_register_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
...
int sc1135_unregister_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
...
int SmartSens_SC1135_init(SENSOR_DO_I2CRD do_i2c_read, 
...
//	af_register_callback(pAfRegister);
	...
	printf("SmartSens SC1135 sensor 1080P30fps init success!\n");
	return s32Ret;
}

int SC1135_get_resolution(uint32_t *ret_width, uint32_t *ret_height)
{
	if(ret_width && ret_height){
		*ret_width = SENSOR_SC1135_WIDTH;
		*ret_height = SENSOR_SC1135_HEIGHT;
		return 0;
	}
	return -1;
}

bool SENSOR_SC1135_probe()
{
	uint16_t ret_data1 = 0;
	uint16_t ret_data2 = 0;
	uint16_t ret_data3 = 0;
	
	SENSOR_I2C_READ(0x580B, &ret_data1);
	SENSOR_I2C_READ(0x3108, &ret_data2);
	SENSOR_I2C_READ(0x3400, &ret_data3);
	
	if(ret_data1 == SC1135_CHECK_DATA_LSB && ret_data2 == SC1135_CHECK_DATA_MSB && ret_data3 == SC1135_CHECK_DATA_HSB){
		//set i2c pinmux
...
int SC1135_get_sensor_name(char *sensor_name)
{
	if(sensor_name != NULL) {
