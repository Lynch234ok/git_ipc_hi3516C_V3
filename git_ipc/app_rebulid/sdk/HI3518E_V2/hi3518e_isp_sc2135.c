...
#define SENSOR_SC2135_WIDTH 1920
#define SENSOR_SC2135_HEIGHT 1080
#define SC2135_CHECK_DATA_LSB	(0x21)
#define SC2135_CHECK_DATA_MSB	(0x35)//CHIP_ID address is 0x3107&0x3108
...
#define CMOS_OV2135_SLOW_FRAMERATE_MODE (0)
...
static HI_S32 sc2135_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID sc2135_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID sc2135_slow_framerate_set(HI_U32 u32FullLines,
...
static HI_VOID sc2135_inttime_update(HI_U32 u32IntTime)
{
...
static HI_VOID sc2135_Again_limit(HI_U32 *pu32Again )
{
		//�������ģ�������߼�?
		HI_U32 u32CurrVal = 0;
...
static HI_VOID sc2135_gains_update(HI_U32 u32Again, HI_U32 u32Dgain)
{
 // 20151222
    ...
    SENSOR_I2C_WRITE(0x3e09, u32Again & 0xFF);
...
#if 1
		if(u32Again < 0x20)// if again<2  0.0625*32=2
			SENSOR_I2C_WRITE(0x3630, 0xc4);
		else
			SENSOR_I2C_WRITE(0x3630, 0x84);
		
		if(u32Again < 0x20)// if again<2  0.0625*32=2
			SENSOR_I2C_WRITE(0x3632, 0x20);
		else
			SENSOR_I2C_WRITE(0x3632, 0x40);
	
		if(u32Again < 0x20)
			SENSOR_I2C_WRITE(0x3635, 0x56);
		else if(u32Again <= 0xF0)
			SENSOR_I2C_WRITE(0x3635, 0x54);
		else 
			SENSOR_I2C_WRITE(0x3635, 0x52);
			
		if(u32Again <= 0xf0){// if again<=16
			SENSOR_I2C_WRITE(0x3620, 0x92);
		}
		else{
		...
		if(u32Again < 0xf0){// if again<16
			SENSOR_I2C_WRITE(0x3315, 0x02);
		}
		else{
			SENSOR_I2C_WRITE(0x3315, 0x00);
		}
		...
		//2016.03.16 sc2135 high temperature 
#if 0
...
HI_S32 sc2135_init_ae_exp_function(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AE_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_ae_default    = sc2135_get_ae_default;
    pstExpFuncs->pfn_cmos_fps_set           = sc2135_fps_set;
    pstExpFuncs->pfn_cmos_slow_framerate_set= sc2135_slow_framerate_set;    
    pstExpFuncs->pfn_cmos_inttime_update    = sc2135_inttime_update;
    pstExpFuncs->pfn_cmos_gains_update      = sc2135_gains_update;
...
static HI_S32 sc2135_get_awb_default(AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
...
HI_S32 sc2135_init_awb_exp_function(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AWB_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_awb_default = sc2135_get_awb_default;
...
*/
HI_U32 sc2135_get_isp_default(ISP_CMOS_DEFAULT_S *pstDef)
{
    if (HI_NULL == pstDef)
    {
...
*/    
 ...
 //   memcpy(&pstDef->stLsc.stLscParaTable[0], &g_stCmosLscTable[0], sizeof(ISP_LSC_CABLI_TABLE_S)*HI_ISP_LSC_LIGHT_NUM);  
  ...
  //  pstDef->stNoiseTbl.u8SensorIndex = HI_ISP_NR_SENSOR_INDEX_SC2135;
 ...
 //   memcpy(&pstDef->stLsc,       &g_stCmosLscTable,       sizeof(ISP_LSC_CABLI_TABLE_S));
    ...
    pstDef->stSensorMaxResolution.u32MaxWidth  = SENSOR_SC2135_WIDTH;
    pstDef->stSensorMaxResolution.u32MaxHeight = SENSOR_SC2135_HEIGHT;

    return 0;
}

HI_U32 sc2135_get_isp_black_level(ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
   // HI_S32  i;
...
HI_VOID sc2135_set_pixel_detect(HI_BOOL bEnable)
{
...
HI_VOID sc2135_set_wdr_mode(HI_U8 u8Mode)
{
    bInit = HI_FALSE;
...
HI_U32 sc2135_get_sns_regs_info(ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
...
static HI_S32 sc2135_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
...
static void sc2135_reg_init(void)
{	//20160128
		...
		SENSOR_I2C_WRITE(0x3300,0x20);	//eq  20160315
		SENSOR_I2C_WRITE(0x3301,0x08);	//cmprst  20160120 20160307
		SENSOR_I2C_WRITE(0x3303,0x30);	// count down 1111 20160120
		SENSOR_I2C_WRITE(0x3306,0x78);	// tx 1111 20160307
		SENSOR_I2C_WRITE(0x330b,0xc2);	// count up
		SENSOR_I2C_WRITE(0x3309,0x30);	//cnt up gap 20160307
		SENSOR_I2C_WRITE(0x3308,0x0a);	//cnt up gap 20160307
		SENSOR_I2C_WRITE(0x331E,0x26);	//integ 1st pos point 20160307
		SENSOR_I2C_WRITE(0x331F,0x26);	//integ 2nd pos point 20160307
		SENSOR_I2C_WRITE(0x3320,0x2c);	//ofs fine 1st pos point 20160307
		SENSOR_I2C_WRITE(0x3321,0x2c);	//ofs fine 2nd pos point 20160307
		SENSOR_I2C_WRITE(0x3322,0x2c);	//20160307
		SENSOR_I2C_WRITE(0x3323,0x2c);	//20160307
		   
		SENSOR_I2C_WRITE(0x330e,0x20);	//memory readout delay 0613 0926
		SENSOR_I2C_WRITE(0x3f05,0xdf);	//counter clock div [3] column fpn 0926
		SENSOR_I2C_WRITE(0x3f01,0x04);	//WRITE TIME
		SENSOR_I2C_WRITE(0x3626,0x04);	//WRITE/READ TIME GAP
		   
		SENSOR_I2C_WRITE(0x3312,0x06);	//prechg tx auto ctrl [5] 
		SENSOR_I2C_WRITE(0x3340,0x04);	//EXP1
		SENSOR_I2C_WRITE(0x3341,0x30);	
		SENSOR_I2C_WRITE(0x3342,0x02);	//EXP1
		SENSOR_I2C_WRITE(0x3343,0x20);	

		   
		SENSOR_I2C_WRITE(0x3333,0x10);	
		SENSOR_I2C_WRITE(0x3334,0x20);	
		SENSOR_I2C_WRITE(0x3621,0x18);	
		SENSOR_I2C_WRITE(0x3626,0x04);	


		SENSOR_I2C_WRITE(0x3635,0x34);	//bypass NVDD analog config  20160113
		SENSOR_I2C_WRITE(0x3038,0xa4);	//enable sa1/ecl blksun  
		SENSOR_I2C_WRITE(0x3630,0x84);	//94 1111  20160113  20160120
		SENSOR_I2C_WRITE(0x3622,0x0e);	//vrhlp voltage 14~24 could be choosed 20160113
		SENSOR_I2C_WRITE(0x3620,0x62);	//comp and bitline current 20160307
		SENSOR_I2C_WRITE(0x3627,0x08);	//sa1 common voltage

		SENSOR_I2C_WRITE(0x3637,0x87);	// 20160307
		SENSOR_I2C_WRITE(0x3638,0x86);	//column fpn 20160307
		SENSOR_I2C_WRITE(0x3034,0xd2);	//20160307
		SENSOR_I2C_WRITE(0x5780,0xff);	//20160307
		SENSOR_I2C_WRITE(0x5781,0x0c);	//20160307
		SENSOR_I2C_WRITE(0x5785,0x10);	//20160307
						 					 
		SENSOR_I2C_WRITE(0x3d08,0x01);	//data output DVP CLK INV//2016.05.24
		SENSOR_I2C_WRITE(0x3640,0x00);	// pad driver
		SENSOR_I2C_WRITE(0x320c,0x04);	//25fps config
		SENSOR_I2C_WRITE(0x320d,0xb0);	// 
		SENSOR_I2C_WRITE(0x3662,0x82);	//data output DVP CLK INV//2016.05.24
		SENSOR_I2C_WRITE(0x335d,0x00);	// pad driver
		SENSOR_I2C_WRITE(0x4501,0xa4);	// pad driver

		SENSOR_I2C_WRITE(0x3333,0x00);	
		SENSOR_I2C_WRITE(0x3627,0x02);	// PLL	67.5M pclk
		SENSOR_I2C_WRITE(0x3620,0x62);	
		SENSOR_I2C_WRITE(0x5781,0x04);
						
		SENSOR_I2C_WRITE(0x3333,0x10);	
		SENSOR_I2C_WRITE(0x3306,0x69);	// PLL	67.5M pclk
		SENSOR_I2C_WRITE(0x3635,0x52);	
		SENSOR_I2C_WRITE(0x3636,0x7c);	
		SENSOR_I2C_WRITE(0x3631,0x84);	//lpDVDD
		SENSOR_I2C_WRITE(0x330b,0xe0);	//0820	20160113  20160120

		SENSOR_I2C_WRITE(0x3637,0x88);	//0820	20160113  20160120
		SENSOR_I2C_WRITE(0x3306,0x6b);	//1018	20160113  20160120
		SENSOR_I2C_WRITE(0x330b,0xc2);	// count up
		SENSOR_I2C_WRITE(0x3630,0x84);	//1018

		//2016.08.16
		SENSOR_I2C_WRITE(0x335d,0x20);	//
		SENSOR_I2C_WRITE(0x3368,0x02);
		SENSOR_I2C_WRITE(0x3369,0x00);	//
		SENSOR_I2C_WRITE(0x336a,0x04);	//
		SENSOR_I2C_WRITE(0x336b,0x65);	//
		SENSOR_I2C_WRITE(0x330e,0x20);	//
		SENSOR_I2C_WRITE(0x3367,0x05);	//

		SENSOR_I2C_WRITE(0x3620,0x92);	//
		SENSOR_I2C_WRITE(0x3634,0xd2);	//
		SENSOR_I2C_WRITE(0x3633,0x17);	//
		SENSOR_I2C_WRITE(0x3315,0x02);	//

		SENSOR_I2C_WRITE(0x3334,0xa0);	//
		SENSOR_I2C_WRITE(0x3312,0x00);	//
		SENSOR_I2C_WRITE(0x335e,0x02);	//
		SENSOR_I2C_WRITE(0x335f,0x0a);	//

		//2016.09.07
		SENSOR_I2C_WRITE(0x3306,0x60);	//
		SENSOR_I2C_WRITE(0x3f05,0xd2);	//	

		SENSOR_I2C_WRITE(0x0100,0x01);	//1018	


//2017.08.16 add for high AE

		SENSOR_I2C_WRITE(0x3625,0x01);	
		SENSOR_I2C_WRITE(0x3624,0x80);	


		
		bSensorInit = HI_TRUE;
		printf("==================================================\n");
		printf("===SC2135 sensor 1080P30fps reg init success!=====\n");
		printf("==================================================\n");

}


HI_S32 sc2135_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    memset(pstSensorExpFunc, 0, sizeof(ISP_SENSOR_EXP_FUNC_S));

    pstSensorExpFunc->pfn_cmos_sensor_init = sc2135_reg_init;
    //pstSensorExpFunc->pfn_cmos_sensor_exit = sensor_exit;
    pstSensorExpFunc->pfn_cmos_sensor_global_init = sc2135_global_init;
    pstSensorExpFunc->pfn_cmos_set_image_mode = sc2135_set_image_mode;
    pstSensorExpFunc->pfn_cmos_set_wdr_mode = sc2135_set_wdr_mode;
    
    pstSensorExpFunc->pfn_cmos_get_isp_default = sc2135_get_isp_default;
    pstSensorExpFunc->pfn_cmos_get_isp_black_level = sc2135_get_isp_black_level;
    pstSensorExpFunc->pfn_cmos_set_pixel_detect = sc2135_set_pixel_detect;
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = sc2135_get_sns_regs_info;
...
int sc2135_register_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
    ...
    sc2135_init_sensor_exp_function(&stIspRegister.stSnsExp);
    ...
    sc2135_init_ae_exp_function(&stAeRegister.stSnsExp);
    ...
    sc2135_init_awb_exp_function(&stAwbRegister.stSnsExp);
...
int sc2135_unregister_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
...
int SC2135_set_mirror(bool mirror){
	...
	if(sensor_mirror == false && sensor_flip == false){//normal
		...
		SENSOR_I2C_WRITE(0x3220,0x06);	
		SENSOR_I2C_WRITE(0x3221,0x00);	
		SENSOR_I2C_WRITE(0x3213,0x11);		
...
int SmartSens_SC2135_init(SENSOR_DO_I2CRD do_i2c_read, 
...
//	af_register_callback(pAfRegister);
	...
	printf("SmartSens SC2135 sensor 1080P30fps init success!\n");
	return s32Ret;	
}


int SC2135_get_resolution(uint32_t *ret_width, uint32_t *ret_height)
{
    if(ret_width && ret_height){
        *ret_width = SENSOR_SC2135_WIDTH;
        *ret_height = SENSOR_SC2135_HEIGHT;
        return 0;
    }
    return -1;
}

bool SENSOR_SC2135_probe()
{
    ...
    if(ret_data1 == SC2135_CHECK_DATA_LSB && ret_data2 == SC2135_CHECK_DATA_MSB){
        //set i2c pinmux
...
int SC2135_get_sensor_name(char *sensor_name)
{
    if(sensor_name != NULL) {
