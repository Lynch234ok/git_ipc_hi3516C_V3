...
#define SENSOR_SC2035_WIDTH 1920
#define SENSOR_SC2035_HEIGHT 1080
#define SC2035_CHECK_DATA_LSB	(0x20)
#define SC2035_CHECK_DATA_MSB	(0x32)//CHIP_ID address is 0x3107&0x3108
...
static HI_S32 sc2035_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID sc2035_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID sc2035_slow_framerate_set(HI_U32 u32FullLines,
...
static HI_VOID sc2035_inttime_update(HI_U32 u32IntTime)
{
...
static HI_VOID sc2035_Again_limit(HI_U32 *pu32Again )
{
		//�������ģ�������߼�?
		HI_U32 u32CurrVal = 0;
...
static HI_VOID sc2035_gains_update(HI_U32 u32Again, HI_U32 u32Dgain)
{
 // 20151222
	//printf("%s......%d......%#x.....%#x......%d.....%#x.....\n",__FUNCTION__,__LINE__, u32Again, u32Dgain, u32TotalAgain, u32RegValul);

	//�������ģ�������߼�?
	sc2035_Again_limit(&u32Again);
    ...
    SENSOR_I2C_WRITE(0x3e09, u32Again & 0xFF);
...
#if 1
		if(u32Again <= 0x20)
			SENSOR_I2C_WRITE(0x3630, 0xb4);
		else
			SENSOR_I2C_WRITE(0x3630, 0x94);
	
		if(u32Again <= 0x20)
			SENSOR_I2C_WRITE(0x3635, 0x6c);
		else if(u32Again <= 0x40)
			SENSOR_I2C_WRITE(0x3635, 0x6a);
		else if(u32Again <= 0x80)
			SENSOR_I2C_WRITE(0x3635, 0x68);
		...
		//2016.03.16 sc2035 high temperature 
#if 0
				SENSOR_I2C_READ(0x3911, &data); 
				//printf("****0x3911:0x%x*****\n",data);
				if(data>=0x15){
					SENSOR_I2C_WRITE(0x3416,0x50);
				}
				else if(data<=0x12){
					SENSOR_I2C_WRITE(0x3416,0x12);
				}
				//SENSOR_I2C_READ(0x3416, &data); 
				//printf("****0x3416:0x%x*****\n",data);
#endif 
...
HI_S32 sc2035_init_ae_exp_function(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AE_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_ae_default    = sc2035_get_ae_default;
    pstExpFuncs->pfn_cmos_fps_set           = sc2035_fps_set;
    pstExpFuncs->pfn_cmos_slow_framerate_set= sc2035_slow_framerate_set;    
    pstExpFuncs->pfn_cmos_inttime_update    = sc2035_inttime_update;
    pstExpFuncs->pfn_cmos_gains_update      = sc2035_gains_update;
...
static HI_S32 sc2035_get_awb_default(AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
...
HI_S32 sc2035_init_awb_exp_function(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AWB_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_awb_default = sc2035_get_awb_default;
...
*/
HI_U32 sc2035_get_isp_default(ISP_CMOS_DEFAULT_S *pstDef)
{
    if (HI_NULL == pstDef)
    {
...
*/    
 ...
 //   memcpy(&pstDef->stLsc.stLscParaTable[0], &g_stCmosLscTable[0], sizeof(ISP_LSC_CABLI_TABLE_S)*HI_ISP_LSC_LIGHT_NUM);  
  ...
  //  pstDef->stNoiseTbl.u8SensorIndex = HI_ISP_NR_SENSOR_INDEX_SC2035;
 ...
 //   memcpy(&pstDef->stLsc,       &g_stCmosLscTable,       sizeof(ISP_LSC_CABLI_TABLE_S));
    ...
    pstDef->stSensorMaxResolution.u32MaxWidth  = SENSOR_SC2035_WIDTH;
    pstDef->stSensorMaxResolution.u32MaxHeight = SENSOR_SC2035_HEIGHT;

    return 0;
}

HI_U32 sc2035_get_isp_black_level(ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
   // HI_S32  i;
...
HI_VOID sc2035_set_pixel_detect(HI_BOOL bEnable)
{
...
HI_VOID sc2035_set_wdr_mode(HI_U8 u8Mode)
{
    bInit = HI_FALSE;
...
HI_U32 sc2035_get_sns_regs_info(ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
...
static HI_S32 sc2035_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
...
static void sc2035_reg_init(void)
{	//20160128
	
		SENSOR_I2C_WRITE(0x3105,0x02);	//start up timing begin
		SENSOR_I2C_WRITE(0x0103,0x01);	// reset all registers
		SENSOR_I2C_WRITE(0x3105,0x02);	
		SENSOR_I2C_WRITE(0x0100,0x00);	//start up timing end
		   
		SENSOR_I2C_WRITE(0x301E,0xB0);	// mode select
			
		SENSOR_I2C_WRITE(0x320c,0x03);	// hts=2000
		SENSOR_I2C_WRITE(0x320d,0xe8);	 
		SENSOR_I2C_WRITE(0x3231,0x24);	// half hts  to 2000
		SENSOR_I2C_WRITE(0x320E,0x04);	
		SENSOR_I2C_WRITE(0x320F,0x65);	
		SENSOR_I2C_WRITE(0x3211,0x08);	//x start 20160113 20160120
		SENSOR_I2C_WRITE(0x3213,0x10);	//y start
		   
		SENSOR_I2C_WRITE(0x3e03,0x03);  //AEC AGC 03 : close aec/agc 00: open aec/agc
		//SENSOR_I2C_WRITE(0x3e03,0x0b);	//AEC AGC 03 : close aec/agc 00: open aec/agc  //barlow 20160510
		SENSOR_I2C_WRITE(0x3e01,0x46);	//exp time
		SENSOR_I2C_WRITE(0x3e08,0x00);	//gain 1x
		SENSOR_I2C_WRITE(0x3e09,0x10);	//10-1f,16step->1/16
		SENSOR_I2C_WRITE(0x3518,0x03);	
		SENSOR_I2C_WRITE(0x5025,0x09);	
		   
		SENSOR_I2C_WRITE(0x3908,0x00);	//BLC RNCincrease blc target for rnc  //barlow 20160512 0xc0->0x50
		SENSOR_I2C_WRITE(0x3907,0x01);	//12.14 //barlow 20160512 0x01->0x00
		SENSOR_I2C_WRITE(0x3662,0x82);
		SENSOR_I2C_WRITE(0x3928,0x01);	//20160315
		SENSOR_I2C_WRITE(0x3416,0x12);	//20160113
		SENSOR_I2C_WRITE(0x3401,0x1e);	//12.11
		SENSOR_I2C_WRITE(0x3402,0x0c);	//12.11
		SENSOR_I2C_WRITE(0x3403,0x70);	//12.11
		SENSOR_I2C_WRITE(0x3e0f,0x90);	//barlow 20160512 0x90->0x14
		   
		SENSOR_I2C_WRITE(0x3638,0x84);	//RAMP config  20160113B
		SENSOR_I2C_WRITE(0x3637,0xbc);	//1018 20160113 20160120
		SENSOR_I2C_WRITE(0x3639,0x98);	
		SENSOR_I2C_WRITE(0x3035,0x01);	//count clk
		SENSOR_I2C_WRITE(0x3034,0xc2);	//1111	 20160120
		   
		SENSOR_I2C_WRITE(0x3300,0x12);	//eq  20160315
		SENSOR_I2C_WRITE(0x3301,0x08);	//cmprst  20160120 20160307
		SENSOR_I2C_WRITE(0x3308,0x30);	// tx 1111 20160307
		SENSOR_I2C_WRITE(0x3306,0x3a);	// count down 1111 20160120
		SENSOR_I2C_WRITE(0x330a,0x00);	
		SENSOR_I2C_WRITE(0x330b,0x90);	// count up
		SENSOR_I2C_WRITE(0x3303,0x30);	//ramp gap 20160307
		SENSOR_I2C_WRITE(0x3309,0x30);	//cnt up gap 20160307
		SENSOR_I2C_WRITE(0x331E,0x2c);	//integ 1st pos point 20160307
		SENSOR_I2C_WRITE(0x331F,0x2c);	//integ 2nd pos point 20160307
		SENSOR_I2C_WRITE(0x3320,0x2e);	//ofs fine 1st pos point 20160307
		SENSOR_I2C_WRITE(0x3321,0x2e);	//ofs fine 2nd pos point 20160307
		SENSOR_I2C_WRITE(0x3322,0x2e);	//20160307
		SENSOR_I2C_WRITE(0x3323,0x2e);	//20160307
		   
		SENSOR_I2C_WRITE(0x3626,0x03);	//memory readout delay 0613 0926
		SENSOR_I2C_WRITE(0x3621,0x28);	//counter clock div [3] column fpn 0926
		SENSOR_I2C_WRITE(0x3F08,0x04);	//WRITE TIME
		SENSOR_I2C_WRITE(0x3F09,0x44);	//WRITE/READ TIME GAP
		SENSOR_I2C_WRITE(0x4500,0x25);	//data delay 0926
		SENSOR_I2C_WRITE(0x3c09,0x08);	// Sram start position
		   
		SENSOR_I2C_WRITE(0x335D,0x20);	//prechg tx auto ctrl [5] 
		SENSOR_I2C_WRITE(0x3368,0x02);	//EXP1
		SENSOR_I2C_WRITE(0x3369,0x00);	
		SENSOR_I2C_WRITE(0x336A,0x04);	//EXP2
		SENSOR_I2C_WRITE(0x336b,0x65);	
		SENSOR_I2C_WRITE(0x330E,0x50);	// start value
		SENSOR_I2C_WRITE(0x3367,0x08);	// end value  12.14
		   
		SENSOR_I2C_WRITE(0x3f00,0x06);	
		SENSOR_I2C_WRITE(0x3f04,0x01);	// sram write
		SENSOR_I2C_WRITE(0x3f05,0xdf);	// 1111  20160113 20160120 20160307
		SENSOR_I2C_WRITE(0x3905,0x1c);	
		   
		SENSOR_I2C_WRITE(0x5780,0x7f);	//DPC
		SENSOR_I2C_WRITE(0x5781,0x0a);	//12.17 20160307
		SENSOR_I2C_WRITE(0x5782,0x0a);	//12.17 20160307
		SENSOR_I2C_WRITE(0x5783,0x08);	//12.17  20160307  20160317
		SENSOR_I2C_WRITE(0x5784,0x08);	//12.17  20160307  20160317
		SENSOR_I2C_WRITE(0x5785,0x18);	//12.11 ; 20160112 20160307
		SENSOR_I2C_WRITE(0x5786,0x18);	//12.11 ; 20160112 20160307
		SENSOR_I2C_WRITE(0x5787,0x18);	//12.11 20160307 20160317
		SENSOR_I2C_WRITE(0x5788,0x18);	// 20160307  20160317
		SENSOR_I2C_WRITE(0x5789,0x01);	//12.11 
		SENSOR_I2C_WRITE(0x578a,0x0f);	//12.11 
		SENSOR_I2C_WRITE(0x5000,0x06);	
						
		SENSOR_I2C_WRITE(0x3632,0x44);	//bypass NVDD analog config  20160113
		SENSOR_I2C_WRITE(0x3622,0x0e);	//enable sa1/ecl blksun  
		SENSOR_I2C_WRITE(0x3627,0x08);	//0921 20160307
		SENSOR_I2C_WRITE(0x3630,0xb4);	//94 1111  20160113  20160120
		SENSOR_I2C_WRITE(0x3633,0x97);	//vrhlp voltage 14~24 could be choosed 20160113
		SENSOR_I2C_WRITE(0x3620,0x62);	//comp and bitline current 20160307
		SENSOR_I2C_WRITE(0x363a,0x0c);	//sa1 common voltage
		SENSOR_I2C_WRITE(0x3333,0x10);	// 20160307
		SENSOR_I2C_WRITE(0x3334,0x20);	//column fpn 20160307
		SENSOR_I2C_WRITE(0x3312,0x06);	//20160307
		SENSOR_I2C_WRITE(0x3340,0x03);	//20160307
		SENSOR_I2C_WRITE(0x3341,0xb0);	//20160307
		SENSOR_I2C_WRITE(0x3342,0x02);	//20160307
		SENSOR_I2C_WRITE(0x3343,0x20);	//20160307
						 
		SENSOR_I2C_WRITE(0x303f,0x81);	//format
		SENSOR_I2C_WRITE(0x501f,0x00);	
		SENSOR_I2C_WRITE(0x3b00,0xf8);	
		SENSOR_I2C_WRITE(0x3b01,0x40);	
		SENSOR_I2C_WRITE(0x3c01,0x14);	
		SENSOR_I2C_WRITE(0x4000,0x00);	
						 
		SENSOR_I2C_WRITE(0x3d08,0x01);	//data output DVP CLK INV//2016.05.24
		SENSOR_I2C_WRITE(0x3640,0x00);	// pad driver
					 
		SENSOR_I2C_WRITE(0x0100,0x01);	
		SENSOR_I2C_WRITE(0x303a,0x07);	// PLL	67.5M pclk
		SENSOR_I2C_WRITE(0x3039,0x8e);	
		SENSOR_I2C_WRITE(0x303f,0x82);	
		SENSOR_I2C_WRITE(0x3636,0x88);	//lpDVDD
		SENSOR_I2C_WRITE(0x3631,0x80);	//0820	20160113  20160120
		SENSOR_I2C_WRITE(0x3635,0x66);	//1018	20160113  20160120
		SENSOR_I2C_WRITE(0x3105,0x04);	//1018
		SENSOR_I2C_WRITE(0x3105,0x04);	//1018
	
		bSensorInit = HI_TRUE;
		printf("===sc2035 sensor 1080P30fps(Parallel port) init success!=====\n");

}


HI_S32 sc2035_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    memset(pstSensorExpFunc, 0, sizeof(ISP_SENSOR_EXP_FUNC_S));

    pstSensorExpFunc->pfn_cmos_sensor_init = sc2035_reg_init;
    //pstSensorExpFunc->pfn_cmos_sensor_exit = sensor_exit;
    pstSensorExpFunc->pfn_cmos_sensor_global_init = sc2035_global_init;
    pstSensorExpFunc->pfn_cmos_set_image_mode = sc2035_set_image_mode;
    pstSensorExpFunc->pfn_cmos_set_wdr_mode = sc2035_set_wdr_mode;
    
    pstSensorExpFunc->pfn_cmos_get_isp_default = sc2035_get_isp_default;
    pstSensorExpFunc->pfn_cmos_get_isp_black_level = sc2035_get_isp_black_level;
    pstSensorExpFunc->pfn_cmos_set_pixel_detect = sc2035_set_pixel_detect;
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = sc2035_get_sns_regs_info;
...
int sc2035_register_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
    ...
    sc2035_init_sensor_exp_function(&stIspRegister.stSnsExp);
    ...
    sc2035_init_ae_exp_function(&stAeRegister.stSnsExp);
    ...
    sc2035_init_awb_exp_function(&stAwbRegister.stSnsExp);
...
int sc2035_unregister_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
...
int SmartSens_SC2035_init(SENSOR_DO_I2CRD do_i2c_read, 
...
//	af_register_callback(pAfRegister);
	...
	printf("SmartSens SC2035 sensor 1080P30fps init success!\n");
	return s32Ret;
}

int SC2035_get_resolution(uint32_t *ret_width, uint32_t *ret_height)
{
    if(ret_width && ret_height){
        *ret_width = SENSOR_SC2035_WIDTH;
        *ret_height = SENSOR_SC2035_HEIGHT;
        return 0;
    }
    return -1;
}

bool SENSOR_SC2035_probe()
{
    ...
    if(ret_data1 == SC2035_CHECK_DATA_LSB && ret_data2 == SC2035_CHECK_DATA_MSB) {
        //set i2c pinmux
		sdk_sys->write_reg(0x200f0040, 0x2);
	    sdk_sys->write_reg(0x200f0044, 0x2);

        sdk_sys->write_reg(0x2003002c, 0xb4001);     // sensor unreset, clk 24MHz, VI 99MHz
...
int SC2035_get_sensor_name(char *sensor_name)
{
    if(sensor_name != NULL) {
