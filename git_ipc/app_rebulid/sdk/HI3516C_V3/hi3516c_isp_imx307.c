...
#define SENSOR_I2C_READ(_add, _ret_data) _i2c_read((_add), 0x34, 2, 1)
#define SENSOR_I2C_WRITE(_add, _data) _i2c_write(_add, (_data), 0x34, 2, 1)

#define SENSOR_IMX307_WIDTH 1920
#define SENSOR_IMX307_HEIGHT 1080
#define IMX307_CHECK_DATA(reg) ((reg&0x6)==0x4)?true:false//CHIP_ID address is 0x31DC
...
static HI_U8 gu8SensorImageMode = IMX307_SENSOR_1080P_30FPS_LINEAR_MODE;
...
static HI_S32 imx307_get_ae_default(AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    switch(genSensorMode)
    {
        ...
        case WDR_MODE_NONE:   
            ...
            pstAeSnsDft->u32MaxAgain = 7876439;
...
static HI_VOID imx307_fps_set(HI_FLOAT f32Fps, AE_SENSOR_DEFAULT_S *pstAeSnsDft)
{
...
static HI_VOID imx307_slow_framerate_set(HI_U32 u32FullLines,
...
static HI_VOID imx307_inttime_update(HI_U32 u32IntTime)
{
    HI_U32 u32Value = 0;
...
static HI_VOID imx307_again_calc_table(HI_U32 *pu32AgainLin, HI_U32 *pu32AgainDb)
{
    int i;
...
static HI_VOID imx307_dgain_calc_table(HI_U32 *pu32DgainLin, HI_U32 *pu32DgainDb)
{
    int i;
...
static HI_VOID imx307_gains_update(HI_U32 u32Again, HI_U32 u32Dgain)
{  
...
static HI_VOID imx307_get_inttime_max(HI_U32 u32Ratio, HI_U32 u32MinTime, HI_U32 *pu32IntTimeMax, HI_U32 *pu32LFMaxIntTime)
{
    return;                                                                                                         
}       
...
static HI_VOID imx307_ae_fswdr_attr_set(AE_FSWDR_ATTR_S *pstAeFSWDRAttr)
{
}


HI_S32 imx307_init_ae_exp_function(AE_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AE_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_ae_default    = imx307_get_ae_default;
    pstExpFuncs->pfn_cmos_fps_set           = imx307_fps_set;
    pstExpFuncs->pfn_cmos_slow_framerate_set= imx307_slow_framerate_set;    
    pstExpFuncs->pfn_cmos_inttime_update    = imx307_inttime_update;
    pstExpFuncs->pfn_cmos_gains_update      = imx307_gains_update;
    pstExpFuncs->pfn_cmos_again_calc_table  = imx307_again_calc_table;
    pstExpFuncs->pfn_cmos_dgain_calc_table  = imx307_dgain_calc_table;
    pstExpFuncs->pfn_cmos_get_inttime_max   = NULL;  
    pstExpFuncs->pfn_cmos_ae_fswdr_attr_set = NULL;  
...
static HI_S32 imx307_get_awb_default(AWB_SENSOR_DEFAULT_S *pstAwbSnsDft)
{
    ISP_DEV IspDev = 0;
...
HI_S32 imx307_init_awb_exp_function(AWB_SENSOR_EXP_FUNC_S *pstExpFuncs)
{
    memset(pstExpFuncs, 0, sizeof(AWB_SENSOR_EXP_FUNC_S));

    pstExpFuncs->pfn_cmos_get_awb_default = imx307_get_awb_default;
...
static ISP_CMOS_YUV_SHARPEN_S g_stIspYuvWdrSharpen = 
{
	...
	/* au8SkinGain */
    {127, 127,  127,  127,  127,  135,  200,  200,  200,  200,  200,  200,  205,  205,  205,  205},
...
HI_U32 imx307_get_isp_default(ISP_CMOS_DEFAULT_S *pstDef)
{
    if (HI_NULL == pstDef)
    {
...
HI_U32 imx307_get_isp_black_level(ISP_CMOS_BLACK_LEVEL_S *pstBlackLevel)
{
    HI_S32  i;
...
HI_VOID imx307_set_pixel_detect(HI_BOOL bEnable)
{
  	return;
    ...
    if (bEnable) /* setup for ISP pixel calibration mode */
    {
        ...
        SENSOR_I2C_WRITE (VMAX_ADDR + 2, (u32FullLines_5Fps & 0x30000) >> 16);

        SENSOR_I2C_WRITE (SHS1_ADDR, u32MaxIntTime_5Fps & 0xFF);
        SENSOR_I2C_WRITE (SHS1_ADDR + 1,  (u32MaxIntTime_5Fps & 0xFF00) >> 8); 
        SENSOR_I2C_WRITE (SHS1_ADDR + 2, (u32MaxIntTime_5Fps & 0x30000) >> 16); 
    ...
    else /* setup for ISP 'normal mode' */
    {
        ...
        gu32FullLines = gu32FullLinesStd;
        SENSOR_I2C_WRITE (VMAX_ADDR, gu32FullLines & 0xFF); 
        SENSOR_I2C_WRITE (VMAX_ADDR + 1, (gu32FullLines & 0xFF00) >> 8); 
        SENSOR_I2C_WRITE (VMAX_ADDR + 2, (gu32FullLines & 0x30000) >> 16);
        bInit = HI_FALSE;
    }
#endif
    return;
...
HI_VOID imx307_set_wdr_mode(HI_U8 u8Mode)                                          
{                                                                                
    bInit = HI_FALSE;  
...
HI_U32 imx307_get_sns_regs_info(ISP_SNS_REGS_INFO_S *pstSnsRegsInfo)
{
    HI_S32 i;
    ...
    if (HI_FALSE == bInit)
    {
		...
		for (i=0; i<g_stSnsRegsInfo.u32RegNum; i++)
		{
			...
			g_stSnsRegsInfo.astI2cData[i].u32AddrByteNum = 2;
...
static HI_S32 imx307_set_image_mode(ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
...
static HI_VOID imx307_global_init()
{     
    gu8SensorImageMode = IMX307_SENSOR_1080P_30FPS_LINEAR_MODE;
    genSensorMode = WDR_MODE_NONE;
    gu32FullLinesStd = IMX307_VMAX_1080P30_LINEAR;
...
void imx307_reg_init()
{
	// Enter Standby
	SENSOR_I2C_WRITE(0x3000, 0x01); 	// Standby mode
	SENSOR_I2C_WRITE(0x3002, 0x01);	// Master mode stop

	// Mode register setting
	SENSOR_I2C_WRITE(0x3005, 0x01);
	SENSOR_I2C_WRITE(0x3007, 0x00);
	SENSOR_I2C_WRITE(0x3009, 0x02);	// 60fps;0x00->120fps
	SENSOR_I2C_WRITE(0x300c, 0x00);
	SENSOR_I2C_WRITE(0x3010, 0x21);
	SENSOR_I2C_WRITE(0x3011, 0x0a);
	SENSOR_I2C_WRITE(0x3014, 0x0f);	// gain
	SENSOR_I2C_WRITE(0x3018, 0x65);	// VMAX
	SENSOR_I2C_WRITE(0x3019, 0x04);
	SENSOR_I2C_WRITE(0x301c, 0x30);	// HMAX;
	SENSOR_I2C_WRITE(0x301d, 0x11); 	// HMAX;
	SENSOR_I2C_WRITE(0x3020, 0x00);	// SHS1
	SENSOR_I2C_WRITE(0x3021, 0x00);	// SHS1
	SENSOR_I2C_WRITE(0x3022, 0x00);	// SHS1
	SENSOR_I2C_WRITE(0x3030, 0x0B);	// RHS1
	SENSOR_I2C_WRITE(0x3031, 0x00);	// RHS1
	SENSOR_I2C_WRITE(0x3032, 0x00);	// RHS1
	SENSOR_I2C_WRITE(0x3024, 0x00);	// SHS2
	SENSOR_I2C_WRITE(0x3025, 0x00);	// SHS2
	SENSOR_I2C_WRITE(0x3026, 0x00);	// SHS2
	SENSOR_I2C_WRITE(0x3045, 0x01);
	SENSOR_I2C_WRITE(0x3046, 0xe1);	// LVDS 4CH;12Bit
	SENSOR_I2C_WRITE(0x305c, 0x18);	//37.125MHz INCK Setting
	SENSOR_I2C_WRITE(0x305d, 0x00);
	SENSOR_I2C_WRITE(0x305e, 0x20);
	SENSOR_I2C_WRITE(0x305f, 0x01);
	SENSOR_I2C_WRITE(0x309e, 0x4a);
	SENSOR_I2C_WRITE(0x309f, 0x4a);
	SENSOR_I2C_WRITE(0x3106, 0x00);	
	SENSOR_I2C_WRITE(0x311c, 0x0e);
	SENSOR_I2C_WRITE(0x3128, 0x04);
	SENSOR_I2C_WRITE(0x313b, 0x41);
	SENSOR_I2C_WRITE(0x315e, 0x1a);	//37.125MHz INCK5 Setting
	SENSOR_I2C_WRITE(0x3164, 0x1a);
	SENSOR_I2C_WRITE(0x3480, 0x49);	//37.125MHz INCK7 Setting
	SENSOR_I2C_WRITE(0x3129, 0x00);	// ADBIT1,12Bit;
	SENSOR_I2C_WRITE(0x317c, 0x00);	// ADBIT2,12Bit;
	SENSOR_I2C_WRITE(0x31ec, 0x0e);	// ADBIT3,12Bit;

	// Standby Cancel
	SENSOR_I2C_WRITE(0x3000, 0x00); 	// standby
	usleep(20000);	// DELAY20mS
	SENSOR_I2C_WRITE(0x3002, 0x00); 	// master mode start
	SENSOR_I2C_WRITE(0x304B, 0x0a); 	// XVSOUTSEL XHSOUTSEL enable output
	usleep(20000);

	bSensorInit = HI_TRUE;
	printf("==============================================================\n");
	printf("===Sony imx307 sensor 1080P30fps(LVDS port) init success!=====\n");
	printf("==============================================================\n");
...
HI_S32 imx307_init_sensor_exp_function(ISP_SENSOR_EXP_FUNC_S *pstSensorExpFunc)
{
    memset(pstSensorExpFunc, 0, sizeof(ISP_SENSOR_EXP_FUNC_S));

    pstSensorExpFunc->pfn_cmos_sensor_init = imx307_reg_init;
    //pstSensorExpFunc->pfn_cmos_sensor_exit = imx307_exit;
    pstSensorExpFunc->pfn_cmos_sensor_global_init = imx307_global_init;
    pstSensorExpFunc->pfn_cmos_set_image_mode = imx307_set_image_mode;
    pstSensorExpFunc->pfn_cmos_set_wdr_mode = imx307_set_wdr_mode;
    
    pstSensorExpFunc->pfn_cmos_get_isp_default = imx307_get_isp_default;
    pstSensorExpFunc->pfn_cmos_get_isp_black_level = imx307_get_isp_black_level;
    pstSensorExpFunc->pfn_cmos_set_pixel_detect = imx307_set_pixel_detect;
    pstSensorExpFunc->pfn_cmos_get_sns_reg_info = imx307_get_sns_regs_info;
 ...
 * callback structure                                                       *
 ****************************************************************************/
int imx307_register_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
    ...
    imx307_init_sensor_exp_function(&stIspRegister.stSnsExp);
    ...
    imx307_init_ae_exp_function(&stAeRegister.stSnsExp);
    ...
    imx307_init_awb_exp_function(&stAwbRegister.stSnsExp);
...
int imx307_unregister_callback(void)
{
    ISP_DEV IspDev = 0;
    HI_S32 s32Ret;
    ALG_LIB_S stLib;
...
static int imx307_set_init(ISP_INIT_ATTR_S *pstInitAttr)
{
    ISP_DEV IspDev = 0;
...
int IMX307_set_mirror(bool mirror)
{
	return 0;
}

int IMX307_set_flip(bool flip)
{
	return 0;
}
...
int IMX307_get_resolution(uint32_t *ret_width, uint32_t *ret_height)
{
    if(ret_width && ret_height){
        *ret_width = SENSOR_IMX307_WIDTH;
        *ret_height = SENSOR_IMX307_HEIGHT;
        return 0;
    }
    return -1;
}
...
bool IMX307_probe()
{
	...
	//return true;
    ret_data1 = SENSOR_I2C_READ(0x31DC, 0);
    if(IMX307_CHECK_DATA(ret_data1)){
		//set i2c pinmux
		sdk_sys->write_reg(0x1204002C, 0x3);/*I2C0_SDA*/
		sdk_sys->write_reg(0x12040030, 0x3);/*I2C0_SCL*/
		
        sdk_sys->write_reg(0x1201002c, 0x0486800d);// sensor unreset, clk 37.125MHz, VI 148.5MHz
        return true;
    }
    return false;
}

int IMX307_get_sensor_name(char *sensor_name)
{
    if(sensor_name != NULL) {
