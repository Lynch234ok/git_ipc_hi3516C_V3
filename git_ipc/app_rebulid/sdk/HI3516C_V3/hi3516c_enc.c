...
static int _hi3516e_vpss_ch_map_vencID[] = {
	
	VPSS_CHN_MAIN_STREAM,// for main stream encode  ID_0
	VPSS_CHN_SUB_STREAM,//for Sub-stream   ID_1
	VPSS_CHN_MAIN_JPEG,//main JPEG    ID_2
	2,//vpss_md_chn  ID_3
	VPSS_CHN_SUB_JPEG,//sub JPEG     ID_4
	5,
	6,
};

static int _hi3516e_venc_ch_map[] = {
	0,
...
typedef struct SDK_ENC_ATTR {
	...
	bool slow_framerate_trigger;
	pthread_t slow_framerate_tid;
...
#define STREAM_H265_ISSET(__stream_h265) (strlen((__stream_h265)->name) > 0)
#define STREAM_H265_CLEAR(__stream_h265) do{ memset((__stream_h265)->name, 0, sizeof((__stream_h265)->name)); }while(0)
...
#define HI3516CV3_SDK_MAX_ABILITY (1920*1080*35)
#define HI3516EV1_SDK_MAX_ABILITY (1920*1080*20)
	...
	}else if(0x3516c300 == chip_id){
		limit_framerate = HI3516CV3_SDK_MAX_ABILITY / (width * height); 	
	}else if(0x3516e100 == chip_id){
		limit_framerate = HI3516EV1_SDK_MAX_ABILITY / (width * height); 	
	}
...
static int hi3518e_enc_resolution(uint32_t width, uint32_t height);

static int do_enc_resolution_limit(uint32_t *width, uint32_t *height)
{
	char buf[16] = "";
	char tmp[5] = "";
	...
	if(NULL != fid){
		...
		fclose(fid);
		fid = NULL;
		sscanf(buf,"%d %4s %d",&width_old,tmp,&height_old);
		
		if(*width != width_old || *height != height_old){
			//SOC_INFO("New resolution %d x %d",streamAttr->width,streamAttr->height);
			ret = hi3518e_enc_resolution(*width, *height);
			if(0 != ret){
				*width = sensor_width_max;
				*height = sensor_height_max;
				return -1;
			}else{
				return 0;
			}
		...
		if(*width > sensor_width_max || *height > sensor_height_max){
			*width = sensor_width_max;
			*height = sensor_height_max;
		}
	}
...
static int enc_create_stream_h264(int vin, int stream, LP_SDK_ENC_STREAM_H264_ATTR h264_attr)
{
	...
	if(vin < HI_VENC_CH_BACKLOG_REF && stream < HI_VENC_STREAM_BACKLOG_REF
		...
		if(!STREAM_H264_ISSET(streamH264Attr)){
			...
			int const vpssChannel = _hi3516e_vpss_ch_map_vencID[venc_ch];
			...
			VENC_ATTR_H264_AVBR_S *const p_h264_avbr = &p_venc_rc_attr->stAttrH264AVbr;
	//		VENC_PARAM_H264_CBR_S h264_cbrv2;
			...
			if(stream == 0){
				do_enc_resolution_limit((uint32_t *)&(streamH264Attr->width), (uint32_t *)&(streamH264Attr->height));//main resolution
			}
			...
			p_venc_attr_h264->u32BufSize  = (stream == 0) ? (768*1024) : (80*1024);


			// legal check
			p_venc_attr_h264->u32BufSize = SDK_ALIGNED_BIG_ENDIAN(p_venc_attr_h264->u32BufSize,64);
...
//			p_venc_attr_h264->u32RefNum = 1;/* 0: default; number of refrence frame*/
			...
			switch(h264_attr->rc_mode){
				default:
				case kSDK_ENC_H264_RC_MODE_VBR://FIX ME !!!!!
				{
					if(h264_attr->_enable_smartP){
						p_venc_rc_attr->enRcMode = VENC_RC_MODE_H264AVBR;
						vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_SMARTP;
						vencChannelAttr.stGopAttr.stSmartP.s32BgQpDelta = 7;
						vencChannelAttr.stGopAttr.stSmartP.s32ViQpDelta = 2;
	//					vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
						p_h264_avbr->u32Gop =  streamH264Attr->gop;
						p_h264_avbr->u32StatTime = 2;//(p_h264_avbr->u32Gop+ streamH264Attr->fps - 1) / streamH264Attr->fps;
						vencChannelAttr.stGopAttr.stSmartP.u32BgInterval = p_h264_avbr->u32Gop * 10;
						p_h264_avbr->u32SrcFrmRate =  vin_capture_attr.fps;
						p_h264_avbr->fr32DstFrmRate = (typeof(p_h264_avbr->fr32DstFrmRate))streamH264Attr->fps;
						//p_h265_avbr->u32AvgBitRate = bps_limit;
						p_h264_avbr->u32MaxBitRate = bps_limit; // 1.5 times of bitrate
					}else{
						p_venc_rc_attr->enRcMode = VENC_RC_MODE_H264AVBR;
						vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
						p_h264_avbr->u32Gop = streamH264Attr->gop;
						p_h264_avbr->u32StatTime = 2;//(p_h264_avbr->u32Gop + streamH264Attr->fps - 1) / streamH264Attr->fps;
						p_h264_avbr->u32SrcFrmRate = u32SrcFrmRate;
						p_h264_avbr->fr32DstFrmRate = (typeof(p_h264_avbr->fr32DstFrmRate))streamH264Attr->fps;
						//p_h264_avbr->u32AvgBitRate = bps_limit;
						p_h264_avbr->u32MaxBitRate = bps_limit; // 1.5 times of bitrate
					}					
					break;
				}
				case kSDK_ENC_H264_RC_MODE_AUTO:
				{
					...
					vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
					p_h264_vbr->u32Gop = streamH264Attr->fps*2;
				...
				case kSDK_ENC_H264_RC_MODE_CBR:
				{
					...
					vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
					p_h264_cbr->u32Gop = streamH264Attr->fps*2;
					...
					p_h264_cbr->u32FluctuateLevel = 1;	
...
//					p_h264_abr->u32Gop = streamH264Attr->fps*2;
...
//					p_h264_abr->u32MaxBitRate = bps_limit * 4 / 3; // 1.5 times of bitrate
				...
				case kSDK_ENC_H264_RC_MODE_FIXQP:
				{
					...
					vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
					p_h264_fixqp->u32Gop = streamH264Attr->fps*2;
	...
	//		SOC_CHECK(HI_MPI_VENC_CreateChn(venc_ch, &vencChannelAttr));
			...
			do{	
...
#if 0
				if(vpssChannel > 0 && vpssChannel < VPSS_MAX_PHY_CHN_NUM){			
					...
					if(stream == 0 && stream_resolu <= vin_resolu ){
						if(fsrcScale != fdstScale){ 				
							...
							/*	printf("to min x=%d y=%d w=%d h=%d\r\n", stCropInfo.stCropRect.s32X, stCropInfo.stCropRect.s32Y,
								...
								stCropInfo.stCropRect.u32Width = (stCropInfo.stCropRect.u32Width%4)?( stCropInfo.stCropRect.u32Width +4 -stCropInfo.stCropRect.u32Width%4): stCropInfo.stCropRect.u32Width;
				...
				if(vpssChannel < VPSS_MAX_PHY_CHN_NUM){
					...
					printf("SetChnMode :vpssChannel:%d\r\n", vpssChannel);
					...
					SOC_CHECK(HI_MPI_VPSS_GetExtChnAttr(vpssGroup,vpssChannel,&vpss_ext_chn_attr));	
					if(vpss_ext_chn_attr.u32Height <= streamH264Attr->height || vpss_ext_chn_attr.u32Width <= streamH264Attr->width  ){
						vpss_ext_chn_attr.s32BindChn = EXT_STREAM_BAND_VPSS_CHANNEL; // bind to vpss 1
					...
					HI_U32 u32Depth = 0;
					SOC_CHECK(HI_MPI_VPSS_SetDepth(vpssGroup,vpssChannel,u32Depth));
...
/*
							...
							stVencRcPara.stParamH264AVbr.u32MaxIprop = 100;
							stVencRcPara.stParamH264AVbr.u32MaxStillQP = 30;
							stVencRcPara.stParamH264AVbr.u32MaxIQp = 45;
							stVencRcPara.stParamH264AVbr.u32MinIQp = 26;
							stVencRcPara.stParamH264AVbr.u32MaxQp = 45;
							stVencRcPara.stParamH264AVbr.u32MinQp = 26;
*/
							if(h264_attr->_enable_smartP){
	//							stVencRcPara.stParamH264AVbr.enQpMapMode = VENC_RC_QPMAP_MODE_MEANQP; 
								stVencRcPara.stParamH264AVbr.s32ChangePos = 90;
								stVencRcPara.stParamH264AVbr.s32MaxReEncodeTimes = 2;
								stVencRcPara.stParamH264AVbr.s32MinStillPercent = 50;
								stVencRcPara.stParamH264AVbr.u32MinStillPSNR = 0;
								stVencRcPara.stParamH264AVbr.u32MinIprop = 1;
								stVencRcPara.stParamH264AVbr.u32MaxIprop = 100;
								stVencRcPara.stParamH264AVbr.u32MaxStillQP = 30;
								stVencRcPara.stParamH264AVbr.u32MaxQp = 40;								
								stVencRcPara.stParamH264AVbr.u32MinQp = 26;
								stVencRcPara.stParamH264AVbr.u32MaxIQp = 40;
								stVencRcPara.stParamH264AVbr.u32MinIQp = 26;

								streamH264Attr->_enable_smartP = HI_TRUE;
							}else{
								//stVencRcPara.u32RowQpDelta = 3;
								stVencRcPara.stParamH264AVbr.u32MaxIprop = 100;
								stVencRcPara.stParamH264AVbr.u32MaxStillQP = 30;
								...
								streamH264Attr->_enable_smartP = HI_FALSE;
							}
						}
						break;
...
static int enc_create_stream_h265(int vin, int stream, LP_SDK_ENC_STREAM_H265_ATTR h265_attr)
{
		...
		if(vin < HI_VENC_CH_BACKLOG_REF && stream < HI_VENC_STREAM_BACKLOG_REF
			...
			if(!STREAM_H264_ISSET(streamH265Attr)){
				...
				VENC_ATTR_H265_AVBR_S *const p_h265_avbr = &p_venc_rc_attr->stAttrH265AVbr;
		//		VENC_PARAM_H264_CBR_S h264_cbrv2;
				// vin attributes
				ST_SDK_VIN_CAPTURE_ATTR vin_capture_attr;
				sdk_vin->get_capture_attr(vin, &vin_capture_attr);
				...
				if(stream == 0){
					do_enc_resolution_limit((uint32_t *)&(streamH265Attr->width), (uint32_t *)&(streamH265Attr->height));
				}	
				...
				sdk_sys->get_chip_id(&chip_id);
				streamH265Attr->fps = enc_stream_limit_fps(streamH265Attr->width, streamH265Attr->height, streamH265Attr->fps, chip_id);	
				...
				SOC_CHECK(HI_MPI_ISP_GetPubAttr(0,&stPubAttr));
				if(stPubAttr.f32FrameRate < vin_capture_attr.fps){
					if(stPubAttr.f32FrameRate < streamH265Attr->fps){
						u32SrcFrmRate = streamH265Attr->fps;
					}else{
						u32SrcFrmRate = (HI_U32)stPubAttr.f32FrameRate;
					}
				}else{
				...
				p_venc_attr_h265->u32BufSize  = (stream == 0) ? (768*1024) : (80*1024);
				
				// legal check
				p_venc_attr_h265->u32BufSize = SDK_ALIGNED_BIG_ENDIAN(p_venc_attr_h265->u32BufSize,64);
				...
				switch(h265_attr->rc_mode){
					...
					case kSDK_ENC_H265_RC_MODE_VBR://FIX ME !!!!!
					{
						if(h265_attr->_enable_smartP){
							p_venc_rc_attr->enRcMode = VENC_RC_MODE_H265AVBR;
							vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_SMARTP;
							vencChannelAttr.stGopAttr.stSmartP.s32BgQpDelta = 7;
							vencChannelAttr.stGopAttr.stSmartP.s32ViQpDelta = 2;
	//						vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
							p_h265_avbr->u32Gop = streamH265Attr->gop;
							p_h265_avbr->u32StatTime = 2;//(p_h265_avbr->u32Gop+ streamH265Attr->fps - 1) / streamH265Attr->fps;
							vencChannelAttr.stGopAttr.stSmartP.u32BgInterval = p_h265_avbr->u32Gop * 10;
							p_h265_avbr->u32SrcFrmRate =  u32SrcFrmRate;
							p_h265_avbr->fr32DstFrmRate = (typeof(p_h265_avbr->fr32DstFrmRate))streamH265Attr->fps;
							//p_h265_avbr->u32AvgBitRate = bps_limit;
							p_h265_avbr->u32MaxBitRate = bps_limit; // 1.5 times of bitrate
						}else{
							printf("\033[34m   not smartP  \033[0m \n");
							p_venc_rc_attr->enRcMode = VENC_RC_MODE_H265AVBR;
							vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
							vencChannelAttr.stGopAttr.stNormalP.s32IPQpDelta = 2;
							p_h265_avbr->u32Gop = streamH265Attr->gop;
					...
					case kSDK_ENC_H265_RC_MODE_AUTO://auto and vbr
					{
						vencChannelAttr.stRcAttr.enRcMode = VENC_RC_MODE_H265VBR;
						vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						p_h265_vbr->u32Gop =  streamH265Attr->fps*2;// streamH265Attr->fps*2;//streamH265Attr->gop;
						p_h265_vbr->u32StatTime = 2;//(p_h265_avbr->u32Gop+ streamH265Attr->fps - 1) / streamH265Attr->fps;
						p_h265_vbr->u32SrcFrmRate = u32SrcFrmRate;
					...
					case kSDK_ENC_H265_RC_MODE_FIXQP:
					{
						p_venc_rc_attr->enRcMode = VENC_RC_MODE_H264FIXQP;
						vencChannelAttr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						p_h265_fixqp->u32Gop = streamH265Attr->fps*2;//streamH265Attr->fps*2;//streamH265Attr->gop;
						p_h265_fixqp->u32SrcFrmRate = u32SrcFrmRate;
				...
				do{
					...
					if(vpssChannel < VPSS_MAX_PHY_CHN_NUM){
						...
						SOC_CHECK(HI_MPI_VPSS_GetExtChnAttr(vpssGroup,vpssChannel,&vpss_ext_chn_attr)); 
						if(vpss_ext_chn_attr.u32Height <= streamH265Attr->height || vpss_ext_chn_attr.u32Width <= streamH265Attr->width  ){
							vpss_ext_chn_attr.s32BindChn = EXT_STREAM_BAND_VPSS_CHANNEL; // bind to vpss 1
							vpss_ext_chn_attr.s32SrcFrameRate = -1;
							vpss_ext_chn_attr.s32DstFrameRate = -1;
							vpss_ext_chn_attr.enPixelFormat = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
				...
				do{
					VENC_RC_PARAM_S stVencRcPara;
					uint32_t u32ThrdI[16] = {0,0,0,0,3,3,5,5,8,8,8,15,20,20,25,25};//{7,7,7,9,12,14,18,25,255,255,255,255};
					uint32_t u32ThrdP[16] = {0,0,0,0,3,3,5,5,8,8,8,15,20,20,25,25};	
					//SOC_CHECK(HI_MPI_VENC_GetRcPara(venc_ch, &stVencRcPara));
					...
					switch(p_venc_rc_attr->enRcMode){
						...
						case VENC_RC_MODE_H265AVBR:
							{							
								...
								if(h265_attr->_enable_smartP){
									stVencRcPara.stParamH265AVbr.enQpMapMode = VENC_RC_QPMAP_MODE_MEANQP;
									stVencRcPara.stParamH265AVbr.s32ChangePos = 90;
									stVencRcPara.stParamH265AVbr.s32MaxReEncodeTimes = 2;
									stVencRcPara.stParamH265AVbr.s32MinStillPercent = 50;
									stVencRcPara.stParamH265AVbr.u32MinStillPSNR = 0;
									stVencRcPara.stParamH265AVbr.u32MinIprop = 1;
									stVencRcPara.stParamH265AVbr.u32MaxIprop = 100;
									stVencRcPara.stParamH265AVbr.u32MaxStillQP = 30;
									stVencRcPara.stParamH265AVbr.u32MaxQp = 40;								
									stVencRcPara.stParamH265AVbr.u32MinQp = 26;
									stVencRcPara.stParamH265AVbr.u32MaxIQp = 40;
									stVencRcPara.stParamH265AVbr.u32MinIQp = 26;
									
									streamH265Attr->_enable_smartP = HI_TRUE;
									
								}else{
									stVencRcPara.stParamH265AVbr.u32MaxIprop = 100;
									stVencRcPara.stParamH265AVbr.u32MaxStillQP = 30;
									stVencRcPara.stParamH265AVbr.u32MaxQp = 45;								
									stVencRcPara.stParamH265AVbr.u32MinQp = 26;
									stVencRcPara.stParamH265AVbr.u32MaxIQp = 45;
									stVencRcPara.stParamH265AVbr.u32MinIQp = 26;
									
									streamH265Attr->_enable_smartP = HI_FALSE;
								}
							}
							break;
	...
	*/
#if defined(HI3516E_V1)		
			  if(streamH265Attr->width * streamH265Attr->height >= 1920*1080){
				  VPSS_LOW_DELAY_INFO_S stLowDelayInfo;
				  int s32Ret = -1;
				  VPSS_CHN_MODE_S stVpssChnMode2;			  
				  SOC_CHECK(HI_MPI_VPSS_GetChnMode(vpssGroup,vpssChannel,&stVpssChnMode2));
...
static int enc_set_stream_SetRcParam(int vin, int stream, int night)
{
	VENC_CHN venc_ch = 0;
	int ret = -1;
	...
	venc_ch = vin * HI_VENC_STREAM_BACKLOG_REF + stream;

	
	ret = HI_MPI_VENC_GetChnAttr(venc_ch,&stChnAttr);	
	if(HI_SUCCESS != ret){
		return -1;
	}

	ret = HI_MPI_VENC_GetRcParam(venc_ch, &stVencRcPara);
	if(HI_SUCCESS != ret){
		return -1;
	}
	...
	switch(stChnAttr.stRcAttr.enRcMode){
		...
		case VENC_RC_MODE_H265AVBR:
			{							
				if(night){	
					if(HI_TRUE == _sdk_enc.attr.h265_attr[vin][stream]._enable_smartP ){
						stVencRcPara.stParamH265AVbr.u32MaxQp = 50;								
						stVencRcPara.stParamH265AVbr.u32MinQp = 28;
						stVencRcPara.stParamH265AVbr.u32MaxIQp = 50;
						stVencRcPara.stParamH265AVbr.u32MinIQp = 28;								
					}else{		
						stVencRcPara.stParamH265AVbr.u32MaxQp = 50; 							
						stVencRcPara.stParamH265AVbr.u32MinQp = 28;
						stVencRcPara.stParamH265AVbr.u32MaxIQp = 50;
		...
		{							
				if(night){///night mode
					...
					if(HI_TRUE == _sdk_enc.attr.h264_attr[vin][stream]._enable_smartP ){
					
						stVencRcPara.stParamH264AVbr.u32MaxQp = 50; 							
						stVencRcPara.stParamH264AVbr.u32MinQp = 28;
						stVencRcPara.stParamH264AVbr.u32MaxIQp = 50;
						stVencRcPara.stParamH264AVbr.u32MinIQp = 28;										
					}else{
						stVencRcPara.stParamH264AVbr.u32MaxQp = 50; 							
						stVencRcPara.stParamH264AVbr.u32MinQp = 29;
						stVencRcPara.stParamH264AVbr.u32MaxIQp = 50;
						stVencRcPara.stParamH264AVbr.u32MinIQp = 29;						
					}
...
static int enc_creat_snapshot_chn(int vin, int vencChannelJPEG, ssize_t width, ssize_t height)
{
	...
	if(vin < HI_VENC_CH_BACKLOG_REF){				
		...
		VENC_CHN_ATTR_S vencChannelAttr;
		VENC_ATTR_JPEG_S *const vencAttrJPEG = &vencChannelAttr.stVeAttr.stAttrJpege;
		MPP_CHN_S mppChannelVPSS;
		MPP_CHN_S mppChannelVENC;
		ST_SDK_VIN_CAPTURE_ATTR vin_capture_attr;
		...
		HI_U32 chnModeWidth = 0,chnModeHeight = 0,extVpssChnWidth = 0,extVpssChnHeight = 0;
		

		sdk_vin->get_capture_attr(vin, &vin_capture_attr);

		if((VENC_MAX_CHN_NUM - 2) == vencChannelJPEG){ /// main stream jpeg
			vpssChannel = VPSS_CHN_MAIN_JPEG;	
			if(vpssChannel < VPSS_MAX_PHY_CHN_NUM){
				chnModeWidth	 = vin_capture_attr.width;
				chnModeHeight 	 = vin_capture_attr.height;
			}else{
				extVpssChnWidth = vin_capture_attr.width;
				extVpssChnHeight = vin_capture_attr.height;
			}
		}else{			
			vpssChannel = VPSS_CHN_SUB_JPEG;//sub stream
			if(vpssChannel < VPSS_MAX_PHY_CHN_NUM){
				chnModeWidth	= width;
				chnModeHeight	 = height;
			}else{//ext vpss chn
				extVpssChnWidth = width;
				extVpssChnHeight = height;
			}
		}

		if(vpssChannel < VPSS_MAX_PHY_CHN_NUM){
#if defined(HI3516C_V1)
			vpssChanneln_attr.s32SrcFrameRate = vin_capture_attr.fps;
			vpssChanneln_attr.s32DstFrameRate = 1;
		
			stVpssChnMode.u32Width		 = chnModeWidth;
			stVpssChnMode.u32Height 	 = chnModeHeight;
			stVpssChnMode.enChnMode 	 = VPSS_CHN_MODE_USER;
			stVpssChnMode.bDouble		 = HI_FALSE;
			stVpssChnMode.enPixelFormat  = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
			...
			SOC_CHECK(HI_MPI_VPSS_SetChnMode(vpssGroup, vpssChannel, &stVpssChnMode));	
			SOC_CHECK(HI_MPI_VPSS_EnableChn(vpssGroup, vpssChannel));
#elif defined(HI3516C_V3)
			//do nothing!!
			SOC_CHECK(HI_MPI_VPSS_EnableChn(vpssGroup, vpssChannel));
#endif
		}else{
			vpss_ext_chn_attr.u32Width = extVpssChnWidth;
			vpss_ext_chn_attr.u32Height = extVpssChnHeight;
			vpss_ext_chn_attr.s32BindChn = EXT_STREAM_BAND_VPSS_CHANNEL; 
			...
			HI_U32 u32Depth = 0;
			SOC_CHECK(HI_MPI_VPSS_SetDepth(vpssGroup,vpssChannel,u32Depth));
			SOC_CHECK(HI_MPI_VPSS_EnableChn(vin,vpssChannel));
		}
		...
		vencAttrJPEG->u32BufSize = vencAttrJPEG->u32MaxPicWidth * vencAttrJPEG->u32MaxPicHeight * 2 / 3;//1.5
...
static int enc_release_stream_h264(int vin, int stream)
{
	...
	//		int const venc_group = venc_ch;
			...
			SOC_CHECK(HI_MPI_VPSS_DisableChn(0,vpssChannel));
			return 0;
		}
	}
	return -1;
...
static int set_stream_h264_fps_gop_rate(int vin, int stream,LP_SDK_ENC_STREAM_H264_ATTR streamH264Attr,
	...
	if((streamH264Attr->_enable_smartP != h264_attr->_enable_smartP)){
			ret = -1;
			set_enable = false;
	}

	if(set_enable){
		if((streamH264Attr->bps != h264_attr->bps) ||(streamH264Attr->fps != h264_attr->fps)  ||
			...
			if((strcmp(streamH264Attr->name ,h264_attr->name) == 0 ) && (streamH264Attr->width == h264_attr->width) &&  (streamH264Attr->height == h264_attr->height)  &&  (streamH264Attr->profile == h264_attr->profile) 
				...
				//it may need to be optimized here

				//Frame rate limit
				uint32_t chip_id;				
				sdk_sys->get_chip_id(&chip_id);
				h264_attr->fps = enc_stream_limit_fps(h264_attr->width, h264_attr->height, h264_attr->fps, chip_id);

				ST_SDK_VIN_CAPTURE_ATTR vin_capture_attr;
				sdk_vin->get_capture_attr(vin, &vin_capture_attr);
						
				if(h264_attr->fps  > vin_capture_attr.fps){
					h264_attr->fps  = vin_capture_attr.fps; // encode framerate must be less than capture framerate
				}

				h264_attr->bps = sdk_venc_bps_limit(streamH264Attr->width, streamH264Attr->height,
				...
				if (s32Ret == HI_SUCCESS){
					if(VENC_RC_MODE_H264CBR == venc_ch_attr.stRcAttr.enRcMode){
						 venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						 venc_ch_attr.stRcAttr.stAttrH264Cbr.u32BitRate = h264_attr->bps;
						 venc_ch_attr.stRcAttr.stAttrH264Cbr.fr32DstFrmRate = h264_attr->fps;
						 venc_ch_attr.stRcAttr.stAttrH264Cbr.u32Gop = h264_attr->fps * 2;
						 venc_ch_attr.stRcAttr.stAttrH264Cbr.u32StatTime = 2;//h264_attr->gop/h264_attr->fps;
					}else if(VENC_RC_MODE_H264AVBR == venc_ch_attr.stRcAttr.enRcMode){
						 venc_ch_attr.stRcAttr.stAttrH264AVbr.u32MaxBitRate = h264_attr->bps;
						 venc_ch_attr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate = h264_attr->fps;
						 venc_ch_attr.stRcAttr.stAttrH264AVbr.u32Gop = h264_attr->gop;
						 venc_ch_attr.stRcAttr.stAttrH264AVbr.u32StatTime = 2;//h264_attr->gop/h264_attr->fps;
						 if(h264_attr->_enable_smartP){
							venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_SMARTP;	
							venc_ch_attr.stGopAttr.stSmartP.u32BgInterval =  venc_ch_attr.stRcAttr.stAttrH264AVbr.u32Gop * 10;
						 }else{
							venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						 }
						...
						venc_ch_attr.stRcAttr.stAttrH264Abr.fr32DstFrmRate = h264_attr->fps;
						venc_ch_attr.stRcAttr.stAttrH264Abr.u32Gop = h264_attr->fps * 2;
						venc_ch_attr.stRcAttr.stAttrH264Abr.u32StatTime = h264_attr->gop/h264_attr->fps;
						
					}*/else if(VENC_RC_MODE_H264FIXQP == venc_ch_attr.stRcAttr.enRcMode){
						 venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
						 venc_ch_attr.stRcAttr.stAttrH264FixQp.fr32DstFrmRate = h264_attr->fps;
						 venc_ch_attr.stRcAttr.stAttrH264FixQp.u32Gop = h264_attr->fps * 2;
					}
		...
		else if((streamH264Attr->bps == h264_attr->bps) &&(streamH264Attr->fps == h264_attr->fps)  &&
				(streamH264Attr->gop == h264_attr->gop)&&
				(strcmp(streamH264Attr->name ,h264_attr->name) == 0 ) && (streamH264Attr->width == h264_attr->width) &&  (streamH264Attr->height == h264_attr->height)  &&  (streamH264Attr->profile == h264_attr->profile) 
				&&(streamH264Attr->rc_mode == h264_attr->rc_mode) && (streamH264Attr->quality == h264_attr->quality) &&  (streamH264Attr->buf_id == h264_attr->buf_id) ){
			ret = 0;
		}
	}
...
static int set_stream_h265_fps_gop_rate(int vin, int stream,LP_SDK_ENC_STREAM_H265_ATTR streamH265Attr,
								LP_SDK_ENC_STREAM_H265_ATTR h265_attr){
		int ret = -1;
		bool set_enable = true;
		if(sdk_enc){
			switch(_sdk_enc.attr.enType[vin][stream]){
		...
		if((streamH265Attr->_enable_smartP != h265_attr->_enable_smartP)){
			ret = -1;
			set_enable = false;
		}

		if(set_enable){
			if((streamH265Attr->bps != h265_attr->bps) ||(streamH265Attr->fps != h265_attr->fps)  ||
												(streamH265Attr->gop != h265_attr->gop)){
				if((strcmp(streamH265Attr->name ,h265_attr->name) == 0 ) && (streamH265Attr->width == h265_attr->width) &&	(streamH265Attr->height == h265_attr->height)  &&  (streamH265Attr->profile == h265_attr->profile) 
					&&(streamH265Attr->rc_mode == h265_attr->rc_mode) && (streamH265Attr->quality == h265_attr->quality) &&  (streamH265Attr->buf_id == h265_attr->buf_id)){
					//it may need to be optimized here
			
					//Frame rate limit
					uint32_t chip_id;				
					sdk_sys->get_chip_id(&chip_id);
					h265_attr->fps = enc_stream_limit_fps(h265_attr->width, h265_attr->height, h265_attr->fps, chip_id);
						
					ST_SDK_VIN_CAPTURE_ATTR vin_capture_attr;
					sdk_vin->get_capture_attr(vin, &vin_capture_attr);
												
					if(h265_attr->fps  > vin_capture_attr.fps){
						h265_attr->fps	= vin_capture_attr.fps; // encode framerate must be less than capture framerate
					}
								
					h265_attr->bps = sdk_venc_bps_limit(streamH265Attr->width, streamH265Attr->height,
						h265_attr->fps, h265_attr->bps);
						
					streamH265Attr->bps = h265_attr->bps;
					streamH265Attr->fps = h265_attr->fps;
					streamH265Attr->gop = h265_attr->gop;
					int s32Ret = -1;
					VENC_CHN_ATTR_S venc_ch_attr;
					int const venc_ch = __HI_VENC_CH(vin, stream);
					s32Ret = HI_MPI_VENC_GetChnAttr(venc_ch, &venc_ch_attr);
					if (s32Ret == HI_SUCCESS){
						if(VENC_RC_MODE_H265CBR == venc_ch_attr.stRcAttr.enRcMode){
							 venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
							 venc_ch_attr.stRcAttr.stAttrH265Cbr.u32BitRate = h265_attr->bps;
							 venc_ch_attr.stRcAttr.stAttrH265Cbr.fr32DstFrmRate = h265_attr->fps;
							 venc_ch_attr.stRcAttr.stAttrH265Cbr.u32Gop = h265_attr->fps * 2;
							 venc_ch_attr.stRcAttr.stAttrH265Cbr.u32StatTime = 2;//h265_attr->gop/h265_attr->fps;
						}else if(VENC_RC_MODE_H265AVBR == venc_ch_attr.stRcAttr.enRcMode){
							 venc_ch_attr.stRcAttr.stAttrH265AVbr.u32MaxBitRate = h265_attr->bps;
							 venc_ch_attr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate = h265_attr->fps;
							 venc_ch_attr.stRcAttr.stAttrH265AVbr.u32Gop = h265_attr->gop;
							 venc_ch_attr.stRcAttr.stAttrH265AVbr.u32StatTime = 2;//h265_attr->gop/h265_attr->fps;
							 
							 if(h265_attr->_enable_smartP){
								venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_SMARTP;
								venc_ch_attr.stGopAttr.stSmartP.u32BgInterval =  venc_ch_attr.stRcAttr.stAttrH265AVbr.u32Gop * 10;
							 }else{
								venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
							 }
						}/*else if(VENC_RC_MODE_H265ABR == venc_ch_attr.stRcAttr.enRcMode){
								
							venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
							venc_ch_attr.stRcAttr.stAttrH265Abr.u32AvgBitRate = h265_attr->bps;
							venc_ch_attr.stRcAttr.stAttrH265Abr.u32MaxBitRate = h265_attr->bps;
							venc_ch_attr.stRcAttr.stAttrH265Abr.fr32DstFrmRate = h265_attr->fps;
							venc_ch_attr.stRcAttr.stAttrH265Abr.u32Gop = h265_attr->fps * 2;
							venc_ch_attr.stRcAttr.stAttrH265Abr.u32StatTime = h264_attr->gop/h265_attr->fps;
													
						}*/else if(VENC_RC_MODE_H265FIXQP == venc_ch_attr.stRcAttr.enRcMode){
							 venc_ch_attr.stGopAttr.enGopMode = VENC_GOPMODE_NORMALP;
							 venc_ch_attr.stRcAttr.stAttrH265FixQp.fr32DstFrmRate = h265_attr->fps;
							 venc_ch_attr.stRcAttr.stAttrH265FixQp.u32Gop = h265_attr->fps * 2;
						}
						s32Ret = HI_MPI_VENC_SetChnAttr(venc_ch,&venc_ch_attr);
						if(s32Ret == HI_SUCCESS){
							ret = 0;
			...
			else if((streamH265Attr->bps == h265_attr->bps) &&(streamH265Attr->fps == h265_attr->fps)  &&
					(streamH265Attr->gop == h265_attr->gop)&&
				(strcmp(streamH265Attr->name ,h265_attr->name) == 0 ) && (streamH265Attr->width == h265_attr->width) &&  (streamH265Attr->height == h265_attr->height)	&&	(streamH265Attr->profile == h265_attr->profile) 
				&&(streamH265Attr->rc_mode == h265_attr->rc_mode) && (streamH265Attr->quality == h265_attr->quality) &&  (streamH265Attr->buf_id == h265_attr->buf_id) ){
			ret = 0;
			}
		}
	...
	if(vin < HI_VENC_CH_BACKLOG_REF && stream < HI_VENC_STREAM_BACKLOG_REF
		...
		if(STREAM_H264_ISSET(streamH264Attr)){
			if(1){//if(streamH264Attr->width != h264_attr->width || streamH264Attr->height != h264_attr->height){
				...
				if(set_stream_h264_fps_gop_rate(vin,stream,streamH264Attr,h264_attr) != 0){
...
static int enc_set_stream_h265(int vin, int stream, LP_SDK_ENC_STREAM_H265_ATTR h265_attr)
{
	if(vin < HI_VENC_CH_BACKLOG_REF && stream < HI_VENC_STREAM_BACKLOG_REF
		...
		if(STREAM_H265_ISSET(streamH265Attr)){
			if(1){//if(streamH264Attr->width != h264_attr->width || streamH264Attr->height != h264_attr->height){
				if(set_stream_h265_fps_gop_rate(vin,stream,streamH265Attr,h265_attr) != 0){
					// width / height adjust need to restart the stream
					SDK_ENC_release_stream(vin, stream);
...
static uint32_t enc_frame_type_convert(int type)
{
	uint32_t ret = kSDK_ENC_BUF_FRAME_UNUSE;
	switch(type){
		case BASE_IDRSLICE:
			ret = kSDK_ENC_BUF_FRAME_BASE_IDRSLICE;
			break;
		case BASE_PSLICE_REFTOIDR:
			ret = kSDK_ENC_BUF_FRAME_BASE_PSLICE_REFTOIDR;
			break;
		case BASE_PSLICE_REFBYBASE:
			ret = kSDK_ENC_BUF_FRAME_BASE_PSLICE_REFBYBASE;
			break;
		case BASE_PSLICE_REFBYENHANCE:
			ret = kSDK_ENC_BUF_FRAME_BASE_PSLICE_REFBYENHANCE;
			break;
		case ENHANCE_PSLICE_REFBYENHANCE:
			ret = kSDK_ENC_BUF_FRAME_ENHANCE_PSLICE_REFBYENHANCE;
			break;
		case ENHANCE_PSLICE_NOTFORREF:
			ret = kSDK_ENC_BUF_FRAME_ENHANCE_PSLICE_NOTFORREF;
			break;
		case ENHANCE_PSLICE_BUTT:
			ret = kSDK_ENC_BUF_FRAME_ENHANCE_PSLICE_BUTT;
			break;
		default:
			break;
	}
		...
		if(HI_SUCCESS == HI_MPI_VENC_Query(venc_ch, &venc_chn_stat)){
			...
			if(venc_chn_stat.u32CurPacks > 0){				
				...
				//		is_keyframe = (H264E_NALU_ISLICE == venc_pack[venc_chn_stat.u32CurPacks-1].DataType.enH264EType);
						is_keyframe = (H264E_NALU_IDRSLICE  == venc_pack[venc_chn_stat.u32CurPacks-1].DataType.enH264EType);
						if(is_keyframe){					
							*ref_counter = 0;
						}else{
						...
						if(1){//if(0 == streamH264Attr->ref_counter || 2 == streamH264Attr->ref_counter){
							...
							attr.h264.frametype = enc_frame_type_convert(venc_stream.stH264Info.enRefType);
					...
					case kSDK_ENC_BUF_DATA_H265:
						//is_keyframe = (HI3521_MAX_VENC_PACK == venc_stream.u32PackCount);
						is_keyframe = (H265E_NALU_IDRSLICE == venc_pack[venc_chn_stat.u32CurPacks-1].DataType.enH265EType);
						if(is_keyframe){					
							*ref_counter = 0;
						}else{
						...
						if(1){
							...
							attr.h265.frametype = enc_frame_type_convert(venc_stream.stH265Info.enRefType);
							_sdk_enc.attr.u64encPTS[vin][stream] = attr.timestamp_us;
...
*/
									...
									/*
											...
											if(fid){
												...
												if(count --){
													fwrite(venc_stream.pstPack[i].pu8Addr + venc_stream.pstPack[i].u32Offset, 1, venc_stream.pstPack[i].u32Len-venc_stream.pstPack[i].u32Offset, fid);
													SOC_INFO("writing %d @ %d", venc_stream.pstPack[i].u32Len-venc_stream.pstPack[i].u32Offset, 1000 - count);
												}else{
													fclose(fid);
													exit(1);
													//fid = NULL;
...
static int enc_snapshot(int vin, enSDK_ENC_SNAPSHOT_QUALITY quality, ssize_t width, ssize_t height, FILE* stream)
{
	...
	if(vin < HI_VENC_CH_BACKLOG_REF){ 	
		...
		SOC_CHECK(HI_MPI_VENC_StartRecvPic(vencChannelJPEG)); // 开启编码通道接收输入图像
		...
		do {
			...
			fd_set rfd_set;
			VENC_CHN_STAT_S venc_chn_stat;
			HI_S32 const venc_jpeg_fd = HI_MPI_VENC_GetFd(vencChannelJPEG); // 获取编码通道对应的设备文件句柄

			FD_ZERO(&rfd_set); // 将set清零使集合中不含任何fd
			FD_SET(venc_jpeg_fd, &rfd_set); // 将fd加入set集合
			...
			if(ret < 0){
				...
				if(FD_ISSET(venc_jpeg_fd, &rfd_set)){ // 测试fd是否在set集合中
					SOC_CHECK(HI_MPI_VENC_Query(vencChannelJPEG, &venc_chn_stat)); // 查询编码通道状态
					// here you must keep the u32CurPacks not zero
					if(venc_chn_stat.u32CurPacks > 0){
						VENC_STREAM_S venc_stream; // 当前帧的码流包个数
						venc_stream.u32PackCount = venc_chn_stat.u32CurPacks; // 当前帧的码流包个数 
						venc_stream.pstPack = (VENC_PACK_S*)alloca(sizeof(VENC_PACK_S) * venc_stream.u32PackCount);
						SOC_CHECK(HI_MPI_VENC_GetStream(vencChannelJPEG, &venc_stream, HI_IO_NOBLOCK)); // 获取编码的码流 
...
static int enc_snapshotToBuf(int vin, enSDK_ENC_SNAPSHOT_QUALITY quality, ssize_t width, ssize_t height, char *buf, int *nLen)
{
    if(NULL == buf || NULL == nLen || 0 >= *nLen)
    {
        SOC_DEBUG("Snapshot select failed!");
        return -1;
    }
            ...
            else
            {
                if(FD_ISSET(venc_jpeg_fd, &rfd_set))
                {
                    ...
                    if(venc_chn_stat.u32CurPacks > 0)
                    {
                        ...
                        tmpLen = _stream_size(venc_stream);
                        if(*nLen >= tmpLen)
                        {
                            for (i = 0; i < venc_stream.u32PackCount; ++i)
                            {
                                memcpy(buf, venc_stream.pstPack[i].pu8Addr, venc_stream.pstPack[i].u32Len);
                                buf += venc_stream.pstPack[i].u32Len;
                            }
                            *nLen = tmpLen;
                        }
                        SOC_CHECK(HI_MPI_VENC_ReleaseStream(vencChannelJPEG, &venc_stream));
	...
	if(canvas){
		...
		if(x < canvas->width && y < canvas->height){
			if(NULL != canvas->pixels){	
				...
				for(i = 0; i < height; ++i){
					uint16_t* pixel_pos = pixels + (i + y) * canvas->width + x;
					for(ii = 0; ii < width; ++ii){
						*pixel_pos++ = pixel_4444;
					}
				}
...
static void enc_release_overlay_canvas(LP_SDK_ENC_VIDEO_OVERLAY_CANVAS canvas)
{
	if(canvas){
		...
		//	free(canvas->pixels);		
			...
			HI_MPI_SYS_MmzFree(canvas->phy_addr,canvas->pixels);
...
{	
	...
	while (HI_FALSE == bExitOverlayLoop){		
		while((handle_num < _sdk_enc.attr.overlay_handle_num) && (HI_FALSE == bExitOverlayRelease) ){			
			...
			pthread_mutex_lock(&_sdk_enc.attr.overlayex_mutex);
			s32Ret = HI_MPI_RGN_GetAttr(Handle_sub, &stRgnAttrSet);
			if(HI_SUCCESS != s32Ret){
				handle_num ++ ;
				pthread_mutex_unlock(&_sdk_enc.attr.overlayex_mutex);
				continue ;	
			...
			stChn.s32DevId = 0;
			stChn.s32ChnId = _hi3516e_venc_ch_map[VpssChn_sub]; // 1
			HI_MPI_RGN_GetDisplayAttr(Handle_sub, &stChn, &stChnAttr);
			
			HI_MPI_VPSS_GetChnCrop(0,VpssChn_sub,&stCropInfo);
			HI_MPI_VPSS_GetChnMode(0,VpssChn_sub,&stVpssMode);
			...
			stMppChn.s32DevId = 0;
			stMppChn.s32ChnId = _hi3516e_venc_ch_map[VpssChn];//1;
			...
			{	
					...
					stOsdReverseInfo.stLumaRgnInfo.pstRegion[k].u32Height))
				{
					...
					if(1 == VpssChn){		
						HI_MPI_RGN_GetAttr(Handle, &stRgnAttrSet);
						stSize.u32Width  = stRgnAttrSet.unAttr.stOverlay.stSize.u32Width;
						stSize.u32Height = stRgnAttrSet.unAttr.stOverlay.stSize.u32Height;
		...
		float x, float y, LP_SDK_ENC_VIDEO_OVERLAY_CANVAS const canvas)
{
	...
	if(NULL != canvas
		...
		VPSS_CROP_INFO_S stCropInfo;	
		HI_S32 VencChn = __HI_VENC_CH(vin, stream);
		...
		for(i = 0; i < HI_VENC_OVERLAY_BACKLOG_REF; ++i){
			...
			if(!overlay->canvas){
				...
				if(1){//if(overlay->region_handle >= 0){
					...
					region_attr.unAttr.stOverlay.u32CanvasNum = 1;
					SOC_CHECK(HI_MPI_RGN_Create(region_vpss_handle, &region_attr));
					
					memset(&mppChannelVENC, 0, sizeof(mppChannelVENC));
					mppChannelVENC.enModId  = HI_ID_VENC;
					mppChannelVENC.s32DevId = 0;
					mppChannelVENC.s32ChnId = VencChn;  
				...
				//	region_ch_attr.unChnAttr.stOverlayChn.stQpInfo.bQpDisable  = HI_FALSE;
					...
					_sdk_enc.attr.vpss_chn[handle_num] = VencChn;
		...
		if(NULL != overlay){			
			...
			while((_sdk_enc.attr.overlay_handle[handle_num] != region_handle)){
				...
				if(handle_num >= _sdk_enc.attr.overlay_handle_num){
					printf("[Func]:%s [Line]:%d [Info]: OVERLAY HANDLE NUM DOES NOT EXIST!\n",  __FUNCTION__, __LINE__);
					pthread_mutex_unlock(&_sdk_enc.attr.overlayex_mutex);
					return -1;
				}
			}				
		//	SOC_CHECK(HI_MPI_RGN_DetachFrmChn(overlay->region_handle, &mppChannelVPSS));			
			ret = HI_MPI_RGN_Destroy(overlay->region_handle);
			if(ret != 0){
					pthread_mutex_unlock(&_sdk_enc.attr.overlayex_mutex);
					return -1;
			}
			...
			if(0 == _sdk_enc.attr.overlay_handle_num){
				memset(_sdk_enc.attr.overlay_handle,0,sizeof(_sdk_enc.attr.overlay_handle));
			}
			pthread_mutex_unlock(&_sdk_enc.attr.overlayex_mutex);
			return 0;
		}
...
static int enc_show_overlay(int vin, int stream, const char* overlayName, bool showFlag)
{
	...
	/*		
		    ...
		    mppChannel.s32ChnId =  _hi3516e_vpss_ch_map_vencID[__HI_VENC_CH(vin, stream)];
	*/		
...
static int hi3518_mpi_init()
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        printf("HI_MPI_SYS_SetConf fail,Error(%#x)\n", s32Ret);
        return s32Ret;
    }
...
extern unsigned int g_PhyAddr;
extern  void * g_VrtAddr;
...
{
	#define MAX_ROM_SIZE		(16*1024*1024 + 256*1024)
...
*/
	...
	if (length < 1024 || length > MAX_ROM_SIZE)
	{
		printf("%s() size = %d too little or too big\n", __FUNCTION__, length);
		return NULL;
	}
	...
	#else
	int ret;
	if(0 == HI_MPI_SYS_MmzAlloc(&g_PhyAddr, &g_VrtAddr, "Update", NULL, length)) // 在用户态分配 MMZ 内存
	{
		printf("HI_MPI_SYS_MmzAlloc Succeed! length = %d\n", length);
		pVirAddr = (char*)g_VrtAddr;
	}
	else
	{
		printf("HI_MPI_SYS_MmzAlloc error! length = %d\n", length);
		return NULL;
	}
	
	if (g_PhyAddr > 0 &&  length > 0)
	{
		ret = FifoSendPhyAddrMapCmd(g_PhyAddr, length);
		if (ret != 0)
		{
			printf("%s() FifoSendPhyAddrMapCmd errorline:%d\n", __FUNCTION__, ret);
			SDK_unmmap(g_VrtAddr, length);
			return NULL;
		}
	}
...
{
	prctl(PR_SET_NAME, "enc_resolution_reboot");
...
static int hi3518e_enc_resolution(uint32_t width, uint32_t height)//main resolution
{	
...
#define EXT_STREAM_BAND_VPSS_CHANNEL_WIDTH (720)
#define EXT_STREAM_BAND_VPSS_CHANNEL_HEIGHT (576)
#elif defined(HI3516E_V1)
#define EXT_STREAM_BAND_VPSS_CHANNEL_WIDTH (640)
#define EXT_STREAM_BAND_VPSS_CHANNEL_HEIGHT (360)
#endif

static int hi3516e_enc_create_public_vpss_chn(int width, int height, int fps, int depth)
{
	VPSS_CHN_ATTR_S vpssChanneln_attr;
	VPSS_CHN_MODE_S stVpssChnMode;
	SOC_CHECK(HI_MPI_VPSS_GetChnMode(0,EXT_STREAM_BAND_VPSS_CHANNEL,&stVpssChnMode));	
	stVpssChnMode.enChnMode 	 = VPSS_CHN_MODE_USER;
	stVpssChnMode.bDouble		 = HI_FALSE;
	stVpssChnMode.enPixelFormat  = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
	...
	vpssChanneln_attr.s32SrcFrameRate = -1;
	vpssChanneln_attr.s32DstFrameRate = -1;
	SOC_CHECK(HI_MPI_VPSS_SetChnAttr(0, EXT_STREAM_BAND_VPSS_CHANNEL, &vpssChanneln_attr));
	SOC_CHECK(HI_MPI_VPSS_SetChnMode(0, EXT_STREAM_BAND_VPSS_CHANNEL, &stVpssChnMode));
	SOC_CHECK(HI_MPI_VPSS_SetDepth(0,EXT_STREAM_BAND_VPSS_CHANNEL,depth));
	SOC_CHECK(HI_MPI_VPSS_EnableChn(0, EXT_STREAM_BAND_VPSS_CHANNEL));
	return 0;
}

static int hi3516e_enc_release_public_vpss_chn()
{
	SOC_CHECK(HI_MPI_VPSS_DisableChn(0,EXT_STREAM_BAND_VPSS_CHANNEL));
	return 0;
}

#define ONESECOND 1000*1000
#define DELAYTIME 100*1000

static void hi3516e_enc_set_slow_framerate_syn_loop(void *arg)
{
	prctl(PR_SET_NAME, "SetSlowFpsLoop");

	ISP_EXP_INFO_S stExpInfo;
	VENC_CHN_ATTR_S vencChannelAttr;
	HI_U32 SrcFrmRate_before = 0;
	HI_U32 SrcFrmRate_after = 0;

	HI_U32 gop_before = 0;
	HI_U32 gop_after = 0;
	HI_U32 actual_expos_fps = 0;

	bool starlight_mode_enable;
	int i = 0;
	int ret = 0;
	int Time_sum = ONESECOND;
	pthread_detach(pthread_self());

	while(_sdk_enc.attr.slow_framerate_trigger){
		if(Time_sum >= ONESECOND){
			Time_sum = 0;

			HI_SDK_ISP_get_starlight_mode(&starlight_mode_enable);
			if(starlight_mode_enable){
				ret = HI_MPI_ISP_QueryExposureInfo(0, &stExpInfo);
				if(ret != HI_SUCCESS){
					continue;
				}
				...
				for(i=0; i <= 1; i++){
					...
					actual_expos_fps = (HI_U32)((stExpInfo.u32Fps + 50) / 100);
				//	SOC_INFO("stExpInfo.u32Fps =%d actual_expInfo_fps=%d fr32DstFrmRate=%d\n",stExpInfo.u32Fps,actual_expos_fps,vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate);

					switch(vencChannelAttr.stRcAttr.enRcMode){
						default:
						case VENC_RC_MODE_H264AVBR:
							SrcFrmRate_before = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32SrcFrmRate;
							gop_before = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop;

							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate ){
								vencChannelAttr.stRcAttr.stAttrH264AVbr.u32SrcFrmRate  = actual_expos_fps;
								//vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop = 10 * vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate; // FIXME
							//	SOC_INFO("u32Gop=%d u32StatTime=%d",vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop,vencChannelAttr.stRcAttr.stAttrH264AVbr.u32StatTime);

							}else{
								vencChannelAttr.stRcAttr.stAttrH264AVbr.u32SrcFrmRate  = vencChannelAttr.stRcAttr.stAttrH264AVbr.fr32DstFrmRate;
								//vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop = 10 * actual_expos_fps;
							//	SOC_INFO("u32Gop=%d ",vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop);

							}
							if(VENC_GOPMODE_SMARTP == vencChannelAttr.stGopAttr.enGopMode ){
								vencChannelAttr.stGopAttr.stSmartP.u32BgInterval =  vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop * 10;
							}else{

							}

							SrcFrmRate_after = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32SrcFrmRate ;	
							gop_after = vencChannelAttr.stRcAttr.stAttrH264AVbr.u32Gop;

						break;

						case VENC_RC_MODE_H264CBR:

							SrcFrmRate_before = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32SrcFrmRate;
						    gop_before = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32Gop;

							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH264Cbr.fr32DstFrmRate){
								vencChannelAttr.stRcAttr.stAttrH264Cbr.u32SrcFrmRate = actual_expos_fps;
								vencChannelAttr.stRcAttr.stAttrH264Cbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32StatTime * vencChannelAttr.stRcAttr.stAttrH264Cbr.fr32DstFrmRate;
							}else{
								vencChannelAttr.stRcAttr.stAttrH264Cbr.u32SrcFrmRate = vencChannelAttr.stRcAttr.stAttrH264Cbr.fr32DstFrmRate;
								vencChannelAttr.stRcAttr.stAttrH264Cbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32StatTime * actual_expos_fps;

							}

							if(VENC_GOPMODE_SMARTP == vencChannelAttr.stGopAttr.enGopMode ){
								vencChannelAttr.stGopAttr.stSmartP.u32BgInterval =  vencChannelAttr.stRcAttr.stAttrH264Cbr.u32Gop * 10;
							}else{

							}

							SrcFrmRate_after = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32SrcFrmRate;
							gop_after = vencChannelAttr.stRcAttr.stAttrH264Cbr.u32Gop;
						break;

						case VENC_RC_MODE_H265AVBR:
							SrcFrmRate_before = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32SrcFrmRate;
							gop_before = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop;

							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate ){
								vencChannelAttr.stRcAttr.stAttrH265AVbr.u32SrcFrmRate  = actual_expos_fps;
								//vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop = 10 * vencChannelAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate;
							//	SOC_INFO("u32Gop=%d u32StatTime=%d",vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop,vencChannelAttr.stRcAttr.stAttrH265AVbr.u32StatTime);

							}else{
								vencChannelAttr.stRcAttr.stAttrH265AVbr.u32SrcFrmRate  = vencChannelAttr.stRcAttr.stAttrH265AVbr.fr32DstFrmRate;
								//vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop = 10 * actual_expos_fps;
							//	SOC_INFO("u32Gop=%d ",vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop);

							}
							if(VENC_GOPMODE_SMARTP == vencChannelAttr.stGopAttr.enGopMode ){
								vencChannelAttr.stGopAttr.stSmartP.u32BgInterval =  vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop * 10;
							}else{

							}

							SrcFrmRate_after = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32SrcFrmRate ;	
							gop_after = vencChannelAttr.stRcAttr.stAttrH265AVbr.u32Gop;

						break;

						case VENC_RC_MODE_H265CBR:

							SrcFrmRate_before = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRate;
						    gop_before = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32Gop;

							if(actual_expos_fps >= vencChannelAttr.stRcAttr.stAttrH265Cbr.fr32DstFrmRate){
								vencChannelAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRate = actual_expos_fps;
								vencChannelAttr.stRcAttr.stAttrH265Cbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32StatTime * vencChannelAttr.stRcAttr.stAttrH265Cbr.fr32DstFrmRate;
							}else{
								vencChannelAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRate = vencChannelAttr.stRcAttr.stAttrH265Cbr.fr32DstFrmRate;
								vencChannelAttr.stRcAttr.stAttrH265Cbr.u32Gop = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32StatTime * actual_expos_fps;

							}

							if(VENC_GOPMODE_SMARTP == vencChannelAttr.stGopAttr.enGopMode ){
								vencChannelAttr.stGopAttr.stSmartP.u32BgInterval =  vencChannelAttr.stRcAttr.stAttrH265Cbr.u32Gop * 10;
							}else{

							}

							SrcFrmRate_after = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32SrcFrmRate;
							gop_after = vencChannelAttr.stRcAttr.stAttrH265Cbr.u32Gop;
						break;

					}
					if(SrcFrmRate_before != SrcFrmRate_after || gop_after != gop_before){
					//	printf("%s frmRate :%d ; gop:%d\n",__FUNCTION__,actual_expos_fps,gop_after);
...
static int enc_slow_framerate_syn_start()
{
    pthread_attr_t pthread_attr;

	if(!_sdk_enc.attr.slow_framerate_tid){
		int ret = 0;
		_sdk_enc.attr.slow_framerate_trigger = true;
        ret = pthread_attr_init(&pthread_attr);
        if(0 == ret)
        {
            pthread_attr_setstacksize(&pthread_attr, 131072);
            pthread_attr_setdetachstate(&pthread_attr, PTHREAD_CREATE_JOINABLE);
            ret = pthread_create(&_sdk_enc.attr.slow_framerate_tid, &pthread_attr, (void *)hi3516e_enc_set_slow_framerate_syn_loop, NULL);
...
static int enc_slow_framerate_syn_stop()
{
	if(_sdk_enc.attr.slow_framerate_tid){
		_sdk_enc.attr.slow_framerate_trigger = false;
		pthread_join(_sdk_enc.attr.slow_framerate_tid, NULL);
		_sdk_enc.attr.slow_framerate_tid = (pthread_t)NULL;
		return 0;
	}
	return -1;
}
...
static stSDK_ENC_HI3521 _sdk_enc =
{
	...
	.api = {
		...
		.snapshotToBuf = enc_snapshotToBuf,
		},
};
...
int SDK_ENC_init()
{
	...
	if(NULL == sdk_enc){
		...
		hi3516e_enc_create_public_vpss_chn(EXT_STREAM_BAND_VPSS_CHANNEL_WIDTH, EXT_STREAM_BAND_VPSS_CHANNEL_HEIGHT, 5, 0);// CHN 1
		
		sdk_enc->creat_snapshot_chn(0, 3, 640, 360);
		// start
		//sdk_enc->start();
		// success to init
		_sdk_enc.attr.slow_framerate_trigger = false;
		_sdk_enc.attr.slow_framerate_tid = (pthread_t)NULL;
		enc_slow_framerate_syn_start();//for sc2235
		return 0;
	}
	return -1;
}
...
{	
	...
	if(sdk_enc){
		...
		sdk_enc->release_snapshot_chn(0, 3);  //sub JPEG
	...
	hi3516e_enc_release_public_vpss_chn();
	//destroy vpss	
...
int SDK_ENC_create_stream(int vin, int stream, LP_SDK_ENC_STREAM_ATTR stream_attr)
{
	if(sdk_enc){
		...
		enc_set_stream_SetRcParam(0,0,HI_SDK_ISP_get_daynight_mode());
...
int	SDK_ENC_SetRcParam(bool night)
{
	int vin = 0;
	int stream = 0;
	
	enc_set_stream_SetRcParam(vin,stream,night);
	return 0;	
}
