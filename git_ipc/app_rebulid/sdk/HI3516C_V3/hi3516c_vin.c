...
{	
	if(vin < HI_VIN_CH_BACKLOG_REF){
		...
		if(kSDK_VIN_MAGIC != md_attr->magic
			...
			md_attr->ref_hblock = 352 / 4;//352 / 4;
			md_attr->ref_vblock = 288 / 4;//288 / 4;
...
///////////////////////////////////////////////////////////////////////////
			do{
				...
				HI_S32  VpssGrp = 0;
				HI_S32 vpss_md_chn = HI_MD_VPSS_CHN;//2;      //for md chn				
				HI_U32 u32Depth = 2;				
				MD_CHN MdChn = 0;
				HI_U32 u32Size = 0;
				HI_U32 i;
				...
				if(vpss_md_chn < VPSS_MAX_PHY_CHN_NUM){
					...
					vpss_ext_chn_attr.s32BindChn = EXT_STREAM_BAND_VPSS_CHANNEL; // bind to public phy vpss 
					vpss_ext_chn_attr.s32SrcFrameRate = vin_capture_attr.fps;
					vpss_ext_chn_attr.s32DstFrameRate = vin_capture_attr.fps/2;
					if(stVpssChnAttr.s32DstFrameRate < 12){
						stVpssChnAttr.s32DstFrameRate = 12;
					}
					vpss_ext_chn_attr.enPixelFormat = PIXEL_FORMAT_YUV_SEMIPLANAR_420;
					vpss_ext_chn_attr.u32Width = ref_width;
					vpss_ext_chn_attr.u32Height = ref_height;
					vpss_ext_chn_attr.enCompressMode = COMPRESS_MODE_NONE;
					SOC_CHECK(HI_MPI_VPSS_SetExtChnAttr(VpssGrp, vpss_md_chn, &vpss_ext_chn_attr));
				...
				stMd.stMdAttr.enAlgMode = MD_ALG_MODE_BG; // 背景法
				stMd.stMdAttr.enSadMode = IVE_SAD_MODE_MB_4X4;//IVE_SAD_MODE_MB_16X16;			
				stMd.stMdAttr.enSadOutCtrl = IVE_SAD_OUT_CTRL_THRESH;
				//stMd.stMdAttr.u16SadThr = 100; //100 * (1 << 1); //100 * (1 << 2); /* ��ֵԽСԽ���� */
				stMd.stMdAttr.u16SadThr = 100;//70;
				...
				stMd.stMdAttr.stCclCtrl.u16InitAreaThr = 10; //u8WndSz * u8WndSz; 初始面积阈值
				stMd.stMdAttr.stCclCtrl.u16Step = u8WndSz;   // 面积阈值增长步长
			//	printf("\n\n\n\n\nu8WndSz = %d\n\n\n\n\n", u8WndSz);
				//
...
///////////////////////////////////////////////////////////////////////////
			...
			SOC_CHECK(HI_MPI_VPSS_DisableChn(0,HI_MD_VPSS_CHN));
...
static int vin_set_cover(int vin, int id, LP_SDK_VIN_COVER_ATTR cover)
{
	if(vin < HI_VIN_CH_BACKLOG_REF && id < HI_VIN_COVER_BACKLOG_REF){
		...
		if(cover_changed){
			...
			if(HI_SUCCESS == HI_MPI_RGN_GetDisplayAttr(rgn_handle, &mpp_chn_viu, &cover_chn_attr)){ // 获取区域的通道显示属性。
				// configurate the ready region
				rgn_new = false;
			}else{
				// create a new region				
				hi3521_mmz_zone_assign(HI_ID_RGN, 0, 0, (0 == vin) ? HI_MMZ_ZONE_NAME0 : HI_MMZ_ZONE_NAME1);
				SOC_CHECK(HI_MPI_RGN_Create(rgn_handle, &rgn_attr));// 创建区域。
			}
			...
			printf("rgn_new = %d \n",rgn_new);
			if(rgn_new){
				SOC_CHECK(HI_MPI_RGN_AttachToChn(rgn_handle, &mpp_chn_viu, &cover_chn_attr));// 将区域叠加到通道上
			}else{
				SOC_CHECK(HI_MPI_RGN_SetDisplayAttr(rgn_handle, &mpp_chn_viu, &cover_chn_attr));// 设置区域的通道显示属性。
			}
			return 0;
		}
	}
...
static void md_loop(void *arg)
{
	...
	int i = 0;
	VIDEO_FRAME_INFO_S stExtFrmInfo;// 定义视频图像帧信息结构体
	...
	VPSS_GRP VpssGrp  = 0;
	VPSS_CHN VpssChn = HI_MD_VPSS_CHN;
	MD_CHN MdChn = 0;
	HI_S32 s32CurIdx = 0;
	...
	POINT_S astPoint[4]; // 用来保存有移动区域的方块的四个角的坐标
	IVE_CCBLOB_S *pstBlob; // 定义连通区域标记的输出信息
...
// Hislicon 
			...
			HI_U16 threshold = 0;
			/*
			VpssGrp 		VPSS GROUP 号。离线模式的取值范围：[0, VPSS_MAX_GRP_NUM)。在线模式的取值范围：0。							输入
			VpssChn 		VPSS 通道号。取值范围：[0, VPSS_MAX_CHN_NUM)。												输入
			pstVideoFrame 	图像信息。 																				输出
			s32MilliSec 	超时时间 																				输入
			*/
			HI_MPI_VPSS_GetChnFrame(VpssGrp,VpssChn,&stExtFrmInfo,s32GetFrameMilliSec); // 用户从通道获取一帧处理完成的图像
			...
			{
				vin_dma_image(&stExtFrmInfo,&stMd.astImg[s32CurIdx],bInstant);// base  astImg[0]   拷贝图像信息到申请的内存
				HI_MPI_VPSS_ReleaseChnFrame(VpssGrp,VpssChn,&stExtFrmInfo);	// 用户释放一帧通道图像
				bFirstFrm = HI_FALSE;
				goto CHANGE_IDX;
			}
			/*
			MdChn 		通道号，有效范围：[0, 63] 													输入
			pstCur 		当前帧图像指针。不能为空														输入
			pstRef 		参考帧图像指针。不能为空														输入
			pstSad 		Sad 指针。根据 pstMdAttr-> enSadOutCtrl，若需要输出则不能为空。						输出
			pstBlob 	区域信息指针。不能为空。														输出
			*/
			HI_IVS_MD_Process(MdChn,&stMd.astImg[s32CurIdx],&stMd.astImg[1 - s32CurIdx],NULL,&stMd.stBlob); // 移动侦测处理。

			pstBlob = (IVE_CCBLOB_S *)stMd.stBlob.pu8VirAddr;
			u16DstWidth = stExtFrmInfo.stVFrame.u32Width;		// 保存图像宽度
			u16DstHeight = stExtFrmInfo.stVFrame.u32Height;		// 保存图像高度
			...
			if(cur_active_time_us - last_active_time_us > 1000000 ||
				...
				if(md_attr->bitmap_mode){
					...
					u16Thr = (HI_U16)(pstBlob->u16CurAreaThr * (md_attr->bitmap.threshold + 0.1) * 400); // 计算阈值

/*
			
												  A0___________________B1
												   |                   |
												   |                   | 
												   |                   | 
												   |                   | 
...
//	vi_tend_start();  //only  when off-line vpss
//Can't support query op when vi_vpss online
	return md_start();
}
