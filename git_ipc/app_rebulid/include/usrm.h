...
#define USRM_PERMIT_RESET_USR (1<<3)
...
typedef struct USRM_I_KNOW_U
{
	...
	USRM_HOW_ABOUT_t (*reset_user)(void);
...
extern int USRM_get_password(const char *name, char *ret_pwd, int ret_size);
...
extern USRM_I_KNOW_U_t* USRM_get_user(const char* username, USRM_I_KNOW_U_t *usrm);
extern void USRM_reset_user_by_root(void);
