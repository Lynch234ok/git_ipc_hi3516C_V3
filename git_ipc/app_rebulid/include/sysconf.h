...
#define SYS_SCHEDULETIME_DAYS 7
#define SYS_SCHEDULETIME_COUNT 8
typedef struct SYS_SCHEDULETIME{
  	SYS_U8_t start_hour;
	SYS_U8_t start_min;
	SYS_U8_t stop_hour;
	SYS_U8_t stop_min;
...
typedef struct MOTION_HANDLE_TYPE
{
	SYS_U64_t handle_type;/*	0x00000000  无处理
										0x00000001  显示器报警	
										0x00000002  蜂鸣器报警
										0x00000004  报警输出
										0x00000008  抓图上传EMAIL*/
	SYS_U64_t alarm_out_channel;//bit for channel
	SYS_SCHEDULETIME_t schedule_time[SYS_SCHEDULETIME_DAYS][SYS_SCHEDULETIME_COUNT];//7天, 最大8段时间
}MOTION_HANDLE_TYPE_t;

#define SYS_MOTION_HANDLE_TYPE
...
typedef struct SYS_VIN_MD
{
	...
	SYS_U8_t bitmap_mask[15][20];
	MOTION_HANDLE_TYPE_t motion_handle_type;
}SYS_VIN_MD_t;
...
#define SYS_VIN_SIZE_WIDTH_HEIGHT(size, ret_width, ret_height) \
	do{\
		...
		}else if(SYS_VIN_SIZE_960P==size.val){\
...
extern void SYSCONF_md_bitmap_switch(uint8_t *psrc_bitmap, int src_width, int src_height, uint8_t *pdst_bitmap, int dst_width, int dst_height);
extern void SYSCONF_set_software_version_ext(char * pVersion);
extern char * SYSCONF_get_software_version(char ** pVersion);
