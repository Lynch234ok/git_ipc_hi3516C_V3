	...
	FwVersion_t version, FwVersionLimit_t version_limit, FwHeader_t* ret_hdr, char* ret_bname)
{
	...
	if(ret_hdr){
		...
		memset(ret_hdr->soc, 0, sizeof(ret_hdr->soc));

		strncpy(ret_hdr->soc, dev_model, sizeof(ret_hdr->soc));
...
int FIRMWARE_make_rom(const char* path, const char* ext_name, char* ret_file)
{
	...
	fw_ver_limit.low_version.major = SWVER_LIMIT_LMAJ;
	fw_ver_limit.low_version.minor = SWVER_LIMIT_LMIN;
	fw_ver_limit.low_version.revision = SWVER_LIMIT_LREV;
	sprintf(fw_ver_limit.low_version.extend, SWVER_LIMIT_LEXT);
	fw_ver_limit.high_version.major = SWVER_LIMIT_HMAJ;
	fw_ver_limit.high_version.minor = SWVER_LIMIT_HMIN;
	fw_ver_limit.high_version.revision = SWVER_LIMIT_HREV;
	sprintf(fw_ver_limit.high_version.extend, SWVER_LIMIT_HEXT);
...
//		firmware_fill_rand(fw_file, fw_sz);
		...
		snprintf(version, sizeof(version), "%d.%d.%d%s", SWVER_MAJ, SWVER_MIN, SWVER_REV, SWVER_DES_EXT);
		snprintf(cmd, sizeof(cmd), "echo \"%s\" > %s/Version\n", version, path);
		printf(cmd);
		system(cmd);
...
//					}
					...
					GET_FILE_SIZE(block_cfg->file, block_size);
					if(block_size > flash_cfg->size){
						FIRMWARE_DUMP("%s %d exceed max size [%d]", block_cfg->file, block_size, flash_cfg->size);
						assert(0);
					}
		...
		strncpy(&(fw_header->md5_sum[32]), fw_md5, 32);
...
void FIRMWARE_dump_rom(const char* rom_file)
{
	...
	if(fid){
		...
		FIRMWARE_DUMP("soc = %s", fw_header->soc);
		...
		fw_md5 = strdup(&fw_header->md5_sum[32]);
	...
	if(_fw_make_cfg){
		...
		for(i = 0; i < _fw_make_cfg->flash_cnt; ++i){
			...
			for(ii = 0; ii < flash_cfg->block_cnt; ++ii){
				...
				int block_size = 0;
				if(block_cfg->mk_bin){
					//printf("block_cfg->file:%s-%d\r\n", block_cfg->file, flash_cfg->size);
					GET_FILE_SIZE(block_cfg->file, block_size);
					if(block_size > flash_cfg->size){
						FIRMWARE_DUMP("%s %d exceed max size [%d]", block_cfg->file, block_size, flash_cfg->size);
						assert(0);
					...
					if(fid){
						...
						printf("offset:%d+%d=%d---%d\r\n",fw_file_offset, block_cfg->flash_offset, fw_file_offset + block_cfg->flash_offset, flash_cfg->size);
