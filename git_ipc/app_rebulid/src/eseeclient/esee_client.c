...
#define FSM_RELOGIN_INTERVAL (60)
#else
...
#define FSM_RELOGIN_INTERVAL (20*60)//20min
#endif
...
static void* esee_fsm(void* arg)
{
	...
	int relogin_interval = FSM_RELOGIN_INTERVAL;
	int sock = 0;
...
//
							...
							//add a timer for relogin--20min
							relogin_interval = FSM_RELOGIN_INTERVAL;
...
//
		...
		//for relogin every 20min
		if(esee_fsm_current > EFSM_LOGIN_RESPONSE){
			if(0 == relogin_interval--){
					esee_fsm_current = EFSM_LOGIN_REQUEST; // relogin
					relogin_interval = FSM_RELOGIN_INTERVAL;
				}
		}
...
void ESEE_CLIENT_setup(const char* server_domain, uint16_t server_port,
	const char* sn_code, const char * dana_device_id, int ch_cnt, const char* vendor, const char* version)
{
	char sn_crypto[128];
	char dana_id_crypto[128];
	char str_para[32];
	...
	if(dana_device_id){
		...
		ESEE_encrypt_ucode((void*)dana_device_id, (void*)dana_id_crypto);
		// install base para
		ESEE_env_set_item_byname("danaid", dana_device_id);
		ESEE_env_set_item_byname("danaid_crypto", dana_id_crypto);
	}
...
//fix me
int ESEE_CLIENT_restart_by_dana(char *dana_device_id)
{
	char dana_id_crypto[128];

	while(1){
		if(_esee_session){
			char *sn = ESEE_env_find_item_byname("sn");
			if(NULL != sn && strlen(sn) > 0){
				break;//goto restart esee thread
			}
		}
