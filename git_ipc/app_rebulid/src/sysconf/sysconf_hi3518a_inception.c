...
int SYSCONF_HI3518A_INCEPTION_reset(SYSCONF_t* sysconf)
{
	int i = 0;
	SYSCONF_IPCAM_t* const sysconf_hi3518 = &sysconf->ipcam;
	SYS_SPEC_t* const spec = &sysconf_hi3518->spec;
	SYS_INFO_t* const info = &sysconf_hi3518->info;
	SYS_GENERIC_t* const generic = &sysconf_hi3518->generic;
	SYS_AIN_t* const ain = sysconf_hi3518->ain;
	SYS_VIN_t* const vin = sysconf_hi3518->vin;
	SYS_PTZ_t* const ptz = sysconf_hi3518->ptz;
	SYS_NETWORK_LAN_t* const lan = &sysconf_hi3518->network.lan;
...
*/
	...
	///////////////////////////////////////////////////////////////////
	// info
	strncpy(info->device_name, "ip-camera", sizeof(info->device_name));
	strncpy(info->device_model, "IPC", sizeof(info->device_model));
	strncpy(info->device_soc, "N18A", sizeof(info->device_model));
	...
	memset(&vin[0].motion_detect.bitmap_mask, 1, sizeof(vin[0].motion_detect.bitmap_mask));
	...
	for(i = 0; i < SYS_SCHEDULETIME_DAYS; i++){
		vin[0].motion_detect.motion_handle_type.schedule_time[i][0].start_hour = 0;
		vin[0].motion_detect.motion_handle_type.schedule_time[i][0].start_min = 0;
		vin[0].motion_detect.motion_handle_type.schedule_time[i][0].stop_hour = 23;
		vin[0].motion_detect.motion_handle_type.schedule_time[i][0].stop_min = 59;
	}
