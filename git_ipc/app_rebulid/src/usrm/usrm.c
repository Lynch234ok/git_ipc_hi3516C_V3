...
static void storage_reset()
{
	if(_p_usr_m){
		...
		// testing account
		//USRM_add_user("user", "user", true, USRM_PERMIT_LIVE|USRM_PERMIT_SETTING|USRM_PERMIT_PLAYBACK, true);
		USRM_add_user("admin", "", true, USRM_PERMIT_LIVE|USRM_PERMIT_SETTING|USRM_PERMIT_PLAYBACK, true);
...
static USRM_HOW_ABOUT_t usrm_reset_user(void)
{
	storage_reset();
	return USRM_GREAT;
}

USRM_I_KNOW_U_t* USRM_get_user(const char* username, USRM_I_KNOW_U_t *usrm)
{
	int i;
	for(i = 0; i < ARRAY_ITEM(_p_usr_m->user); ++i){
		USER_HOURSE_t* const user_house = _p_usr_m->user + i;
		if(user_house->house_card){
			if(STR_CASE_THE_SAME(username, user_house->username)){
				// matching this user
				struct USRM_WHO_AM_I* const who_am_i = (struct USRM_WHO_AM_I*)(usrm + 1);

				usrm->add_user = usrm_add_user;
				usrm->del_user = usrm_del_user;
				usrm->edit_user = usrm_edit_user;
				usrm->set_password = usrm_set_password;
				usrm->dump = usrm_dump;
				usrm->reset_user = NULL;

				// know which is my house
				who_am_i->house = user_house;

				// public attribute
				usrm->forbidden_zero = 0;
				usrm->username = who_am_i->house->username;
				usrm->password = who_am_i->house->password;
				usrm->is_admin = who_am_i->house->is_admin;
				usrm->permit_flag = who_am_i->house->permit_flag;
...
USRM_I_KNOW_U_t* USRM_login(const char* who_r_u, const char* what_s_password)
{
	int i = 0;
	if(_p_usr_m){
		if(STR_CASE_THE_SAME(who_r_u, _p_usr_m->first_username)
					&& STR_THE_SAME_N(what_s_password, _p_usr_m->first_password, strlen(_p_usr_m->first_password))){
			...
			i_know_u->reset_user = usrm_reset_user;

			// public attribute
			i_know_u->forbidden_zero = 0;
			i_know_u->username = _p_usr_m->first_username;
			i_know_u->password = _p_usr_m->first_password;
			i_know_u->is_admin = 1;
			i_know_u->permit_flag = USRM_PERMIT_ALL;
...
int USRM_get_password(const char *name, char *ret_pwd, int ret_size)
{
	int i = 0;

	// TODO: simple debug: if the first char was zero, so username was invalid, then return -1
	int id = 0 < name[0] ? find_user_house(name) : -1;

	if(0 <= id)
	{
		// TODO: copy password string, it calculates 32 times
		for(i = 0; ret_size > i && sizeof(_p_usr_m->user[id].password) > i; i++)
		{
			ret_pwd[i] = _p_usr_m->user[id].password[i];
		}
		return 0;
	}
	return -1;
...
void USRM_reset_user_by_root(void)
{
	usrm_reset_user();
}

int USRM_Users_Get_Count(){
	return _p_usr_m->n_user;
}

int USRM_Users_Get_User_PWD(int n_user, char*what_name,int name_len, char* pwd,int pwd_len){
	if((NULL == what_name) ||(NULL == pwd)){
		return -1;
	}
	USER_HOURSE_t* const user_tmp = _p_usr_m->user +n_user;
	if(user_tmp){
		strncpy(what_name,user_tmp->username,name_len);
		strncpy(pwd,user_tmp->password,pwd_len);
	}
	return 0;
}
