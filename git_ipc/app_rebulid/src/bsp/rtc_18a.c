...
int RTC_gettime(rtc_time_t *rtc_time)
{
	int rtc_fd;

	rtc_fd = open(RTC_FILE, O_RDWR);
	if (rtc_fd < 0) {
		printf("%s:%d can't open RTC device: %s!\n", __FUNCTION__, __LINE__, RTC_FILE);
		return -1;
	}

	ioctl(rtc_fd, HI_RTC_RD_TIME, rtc_time);
	printf("%s:%d Get RTC: %04d/%02d/%02d %02d:%02d:%02d\n", __FUNCTION__, __LINE__,
...
int RTC_settime(time_t curtime)
{
	...
	printf("%s:%d Timestamp to set: %ld\n",
		   __FUNCTION__, __LINE__, curtime);
	...
	rtc_time.weekday = cur_tm->tm_wday;

	ioctl(rtc_fd, HI_RTC_SET_TIME, &rtc_time);
...
int RTC_sync_to_system()
{
    ...
    memset(&curtime, 0, sizeof(rtc_time_t));
    ...
    ret = ioctl(rtc_fd, HI_RTC_RD_TIME, &curtime);
    if(ret < 0) {
        printf("%s %d rtc read time failed\n", __FUNCTION__, __LINE__);
        goto FAIL_RETURN;
    }
	printf("%s:%d Cur RTC time: %04d/%02d/%02d %02d:%02d:%02d\n", __FUNCTION__, __LINE__,
		   curtime.year, curtime.month, curtime.date,
		   curtime.hour, curtime.minute, curtime.second);

    /* 这里逻辑主要用于有rtc但是没电池的设备，断电重启后，
       如果是1970年就不同步这个时间到系统时间 */
    if(curtime.year == 1970) {
        printf("%s %d sysnc fail,rtc curtime.year %d\n", __FUNCTION__, __LINE__, curtime.year);
        goto FAIL_RETURN;
    }
	...
	time_t timet = timegm(&time_tm);

	tv.tv_sec = timet;
	tv.tv_usec = 0;
	ret = settimeofday(&tv,NULL);
	if(ret < 0){
		printf("%s %d rtc sync to system failed\n", __FUNCTION__, __LINE__);
        goto FAIL_RETURN;
	}
