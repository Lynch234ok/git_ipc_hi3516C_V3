...
/* 帧序号限定 */
#define P2PSDK_MIN_FRAME_SEQ          0x0001
#define P2PSDK_MAX_FRAME_SEQ          0xFFFF
...
struct P2PSDKDevice {
     ...
     * @param ctx               用户上下文
     * @param session           连接会话
     * @param authed            会话是否经过设备授权（OnAuth认证）
     * @param channel           虚拟通道号
     * @param data              数据
 ...
 *                          失败  -1
 */
KP2P_API int P2PSDKSetLogPath(const char *path);


/**
 * @brief 设置日志输出路径
 * @param path              路径
 * @param log_size          日志文件限制大小
 * @return                  成功  0
 *                          失败  -1
 */
KP2P_API int P2PSDKSetLogPath2(const char *path, int log_size);
 ...
 */
enum {
    KP2P_DEV_CAPABILITY_LIGHT_ONOFF = 1,
    KP2P_DEV_CAPABILITY_LIGHT_BRIGHTNESS,
    KP2P_DEV_CAPABILITY_LIGHT_RGB,
    KP2P_DEV_CAPABILITY_LIGHT_TEMPERATUR,
    KP2P_DEV_CAPABILITY_CLOUD_RECORD_ALY,
    KP2P_DEV_CAPABILITY_FISHEYE,
    KP2P_DEV_CAPABILITY_RECORD_ONOFF,
    KP2P_DEV_CAPABILITY_CLOUD_RECORD_QN,
    KP2P_DEV_CAPABILITY_CLOUD_RECORD_AWS,
};
 ...
 * @brief
 * @param p_device
 * @param capabilities  设备能力集，json 串，多个值使用数组，例如: "[1,5]" 表示支持阿里云存的灯泡
 * @return
 */

KP2P_API int P2PSDKInitV2(struct P2PSDKDevice  *p_device, const char *capabilities);
 ...
 *                          失败  -1
 */
KP2P_API int P2PSDKSetEseeServerAddr(const char *server, uint32_t port);
 ...
 *                          失败
 */
KP2P_API ssize_t P2PSDKSendAlarmMsg(void *session, kp2p_frame_head_t *header_msg, void *buf_msg,  size_t buf_len);


/**
 * @brief 虚拟通道数据发送
 * @param session           连接会话
 * @param channel           虚拟通道ID
 * @param data              数据
 * @param data_size         数据长度
 * @return                  成功  0
 *                          失败  -1
 */
KP2P_API int P2PSDKVconSend(void *session, uint32_t channel, const void *data, uint32_t data_size);
