...
SPOOK_SESSION_PROBE_t HTTPD_probe(const void* msg, ssize_t msg_sz)
{
	...
	if((prefix = "GET http://192.168.2.45:80/livestream/", 0 == strncasecmp(prefix, msg, strlen(prefix)))
		|| (prefix = "GET /cgi-bin/hi3510/", 0 == strncasecmp(prefix, msg, strlen(prefix)))){
		printf("$$$$$$$$ Using Old HTTP server temprarily");
