...
int sip_pack_msg(pSipSession s,int MsgType,void *data)
{
	...
	{
		char hb_buf[256];
		sprintf(hb_buf,hb_xml,SIP_ENV_get(s->env,"UserId"));
		sprintf(s->heartbeat_callid,"%s",RAND());
		sprintf(s->sendbuf,hb_fmt,"MESSAGE",
				SIP_ENV_get(s->env,"SipServerId"),
				SIP_ENV_get(s->env,"Realm"),
				SIP_ENV_get(s->env,"DevIp"),
				SIP_ENV_get(s->env,"DevPort"),
				RAND(),
				SIP_ENV_get(s->env,"UserId"),
				SIP_ENV_get(s->env,"Realm"),
				RAND(),
				SIP_ENV_get(s->env,"SipServerId"),
				SIP_ENV_get(s->env,"Realm"),
	...
	{		
		sprintf(s->sendbuf,response_200_fmt,
				...
				SIP_ENV_get(s->env,"DevPort"),s->recv_siphead.sh_Via.branch,SIP_ENV_get(s->env,"DevIp"),
				...
				SIP_ENV_get(s->env,"UserAgent"),0);
	}
	else if(MSG_AUTH == MsgType)
	{
		sprintf(s->sendbuf,auth_fmt,"REGISTER",
				SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"),
				SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),
				RAND(),
				1,"REGISTER",
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),
				SIP_ENV_get(s->env,"UserAgent"),
				0,0);
	...
	{			
		...
		char uri[128];
		sprintf(uri,"sip:%s@%s",SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"));
		HTTP_AUTH_setup(authinfo,SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"UserPwd"),
				uri,"REGISTER",s->auth_key,sizeof(s->auth_key));	

		sprintf(s->sendbuf,re_auth_fmt,"REGISTER",
				SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"),
				SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),s->send_siphead.sh_From.tag,
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),			
	...
	else if(MSG_CATALOG == MsgType)
	{
		...
		sprintf(dev_info,dev_fmt,SIP_ENV_get(s->env,"UserId"),
								 SIP_ENV_get(s->env,"UserAgent"),
								 SIP_ENV_get(s->env,"Manufacturer"),
								 SIP_ENV_get(s->env,"Model"));
		sprintf(catalog,catalog_xml_fmt,SIP_ENV_get(s->env,"UserId"),dev_info);	
		sprintf(s->sendbuf,catalog_response_fmt,"MESSAGE",
				s->recv_siphead.sh_From.id,s->recv_siphead.sh_From.realm,			
				//SIP_ENV_get(s->env,"UserId"),s->recv_siphead.sh_From.realm,			
				SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),RAND(),				
				//SIP_ENV_get(s->env,"UserId"),s->recv_siphead.sh_From.realm,
				s->recv_siphead.sh_From.id,s->recv_siphead.sh_From.realm,
	...
	else if (MSG_INVITE_200 == MsgType)
	{
		...
		sprintf(sdp_buf,invite_200_sdp_fmt,
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"DevIp"),
				SIP_ENV_get(s->env,"UserAgent"),
				SIP_ENV_get(s->env,"DevIp"),
				srtp->data.port,
				SIP_ENV_get(s->env,"UserId"),
				SIP_ENV_get(s->env,"UserPwd"),
				SDP_get_ssrc(sdp));
		...
		sprintf(s->sendbuf,invite_200_fmt,
				...
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),
				"application/SDP",				
				SIP_ENV_get(s->env,"UserAgent"),strlen(sdp_buf),sdp_buf);
	}
	else if(MSG_DEV_CTRL == MsgType)
	{
		char xml_buf[256];
		char *xml_str = strstr(s->recvbuf,"\r\n\r\n");
		sprintf(xml_buf,dev_ctrl_xml_fmt,s->get_tagvalue(xml_str,"SN"),SIP_ENV_get(s->env,"UserId"));
		sprintf(s->sendbuf,record_fmt,
				"MESSAGE",SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"),
				SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),RAND(),
				SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"),
				RAND(),20,"MESSAGE","Application/MANSCDP+xml",
	...
	else if(MSG_DEV_INFO == MsgType)
	{
		...
		sprintf(xml_buf,dev_info_fmt,
				s->get_tagvalue(xml_str,"SN"),SIP_ENV_get(s->env,"UserId"),
				SIP_ENV_get(s->env,"Manufacturer"),SIP_ENV_get(s->env,"Model"),
				SIP_ENV_get(s->env,"Firmware"),SIP_ENV_get(s->env,"MaxCamera"),
				SIP_ENV_get(s->env,"MaxAlarm"));
		sprintf(s->sendbuf,record_fmt,
				"MESSAGE",SIP_ENV_get(s->env,"SipServerId"),SIP_ENV_get(s->env,"Realm"),
				SIP_ENV_get(s->env,"DevIp"),SIP_ENV_get(s->env,"DevPort"),RAND(),
				SIP_ENV_get(s->env,"UserId"),SIP_ENV_get(s->env,"Realm"),RAND(),
	...
	else if(MSG_ALARM == MsgType)
	{
		...
		sprintf(xml_buf,alarm_info_fmt,
				SIP_ENV_get(s->env,"AlarmID"),s->get_systime());
...
void* sip_auth_loop(void* session)
{
	...
	int auth_re_cnt;
	int sleep_time = 0;
	int local_loop_cnt = ++s->run_auth_loop;
	//GB_NOTIFY("new auth_loop@(0x%08x)\n",pthread_self());
...
AUTH_AGAIN:
	...
	for(;;)
	{
		...
		if(time(NULL) - cur_time >= 3)
		{
			if(3 == auth_re_cnt++){
				sleep(5);	
				TRACE("TRY TO LOGIN SIP SERVER AGAIN status:%d\n",s->auth_status);
				goto AUTH_AGAIN; 
				}
			cur_time = time(NULL);
	...
	//until login_alivetime elapse
	if(s->login_out > 30){	
		sleep(s->login_out - 10);
		if( local_loop_cnt != s->run_auth_loop){GB_NOTIFY("exit_auth_loop\'Die Young\'@(0x%08x)\n",pthread_self());return -1;}
		GB_NOTIFY("Alivetime is elapse(0x%08x,0x%08x)\n",getpid(), pthread_self());
		s->online = 0;
		goto AUTH_AGAIN;
		...
		sleep(s->login_out - 1);
		if( local_loop_cnt != s->run_auth_loop){GB_NOTIFY("exit_auth_loop\'Die Young\'@(0x%08x)\n",pthread_self());return -1;}
		GB_NOTIFY("Alivetime elapse(0x%08x,0x%08x)\n",getpid(), pthread_self());
		s->online = 0;
		goto AUTH_AGAIN;
		}
	return 0;
...
{
	if(login){
		s->login_out = atoi(SIP_ENV_get(s->env,"AliveTime"));
		GB_NOTIFY("START login SIPServer(%s:%s),alivetime:%d,heartbeat:%s\n",
						SIP_ENV_get(s->env,"SipServerIp"),
						SIP_ENV_get(s->env,"SipServerPort"),
						s->login_out,SIP_ENV_get(s->env,"HeartBeatTime"));
		s->online = 0;
	...
	int timeout_cnt = 5*1000;
	while(AUTH_S_SUCCESS!=s->auth_status && timeout_cnt != 0)
	{
		usleep(10);
		timeout_cnt -= 10;
	}
	
	if( 0 == timeout_cnt  && AUTH_S_SUCCESS!=s->auth_status)
	{		
		GB_NOTIFY("Be sure u have the right confiration ,or the SipServer is online\n");
	}
	return 0;
}
...
{	
	...
	s->sock = create_sock(NULL,atoi(SIP_ENV_get(s->env,"DevPort")));
	s->server.sin_family = AF_INET;
	s->server.sin_port = htons(atoi(SIP_ENV_get(s->env,"SipServerPort")));
	s->server.sin_addr.s_addr = inet_addr(SIP_ENV_get(s->env,"SipServerIp"));

	s->online = 0;
	s->alarm_status = 0;
	s->run_msg_loop = 0;
	s->heartbeat_time = atoi(SIP_ENV_get(s->env,"HeartBeatTime"));
