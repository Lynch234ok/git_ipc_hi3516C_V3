...
void* gb28181_heartbeat_loop(void *session)
{
	...
	s->heartbeat_valid = 1;// 1 valid heartbeat,0 miss a heartbeat
	time_t cur_time = time(NULL);	
	for(;;)
	{
		usleep(1);
		if(time(NULL)- cur_time > s->heartbeat_time){//send a heartbeat
			if(0 == s->heartbeat_valid)
			{
				GB_NOTIFY("GB Heartbeat timeout@(0x%08x)\n",pthread_self());
				s->auth(s,1);
				return NULL;//now no need this heartbeat loop
			}
			else
			{
				cur_time = time(NULL);
...
exit_msg_loop:
	s->run_msg_loop = 0;//when the msg_loop direct exit by ptz etc.
...
#define BUF_SIZE	(1024*1024)
	...
	while(*trigger){
		...
		if(entries > 0){
			// init mediabuf
			if(media_user == NULL){
				media_id = MEDIABUF_lookup_byname("720p.264");
				if(media_id >= 0){
...
{	
	...
	else if(0 == strncmp(s->recv_siphead.sh_EchoLine.msg_code,"200",3))
	{
		...
		{//this is auth success			
			char time_str[32];
			char time_cmd[32];
			/*set it be state SUCCESS before set the systime,so won't bring out the 
			   resend the re_auth package even the auth is success*/
			...
			sprintf(time_cmd,"date -s \"%s\"",time_str);
			NK_SYSTEM(time_cmd);			
			GB_NOTIFY("200:::AUTH @%s\n",s->get_systime());
//			s->auth_status = AUTH_S_SUCCESS;
	...
	else if(0 == strncmp(s->recv_siphead.sh_ReqLine.cmd,"INVITE",strlen("INVITE")))
	{
	...
	else if(0 == strncmp(s->recv_siphead.sh_ReqLine.cmd,"ACK",strlen("ACK")))
	{
...
void* gb28181_recv(void *session)
{
	pSipSession s = (pSipSession)session;
	fd_set rfds;
	socklen_t sock_len = sizeof(s->server);
	for(;;) 
	{
...
{
	if(0 == cmd){
		GB_NOTIFY("Stop ptz ctrl\n");
	}else{
		switch(cmd)
		{
			case SIP_DEV_CTRL_RIGHT:
				GB_NOTIFY("PTZ:%s speed:%d\n","Right",speed);
				break;
			case SIP_DEV_CTRL_LEFT:
				GB_NOTIFY("PTZ:%s speed:%d\n","Left",speed);
				break;
			case SIP_DEV_CTRL_DOWN:
				GB_NOTIFY("PTZ:%s speed:%d\n","Down",speed);
				break;
			case SIP_DEV_CTRL_UP:
				GB_NOTIFY("PTZ:%s speed:%d\n","Up",speed);
				break;
			case SIP_DEV_CTRL_ZOOM_IN:
				GB_NOTIFY("PTZ:%s speed:%d\n","ZOOM_IN",speed);
				break;
			case SIP_DEV_CTRL_ZOOM_OUT:
				GB_NOTIFY("PTZ:%s speed:%d\n","ZOOM_OUT",speed);
				break;
			default:
				break;				
...
{	
	switch(cmd)
			{
				case SIP_DEV_CTRL_RECORD_START:
					GB_NOTIFY("Device Ctrl:%s arg:%s\n","StartRecord",arg);
					break;
				case SIP_DEV_CTRL_RECORD_STOP:
					GB_NOTIFY("Device Ctrl:%s arg:%s\n","StopRecord",arg);
					break;
				case SIP_DEV_CTRL_GUARD_SET:
					GB_NOTIFY("Device Ctrl:%s arg:%s\n","SetGuard",arg);
					break;
				case SIP_DEV_CTRL_GUARD_RESET:
					GB_NOTIFY("Device Ctrl:%s arg:%s\n","ResetGuard",arg);
					break;
				case SIP_DEV_CTRL_ALARM_RESET:
					GB_NOTIFY("Device Ctrl:%s arg:%s\n","ResetAlarm",arg);
					break;
				default:
					break;				
...
static int gb28181_write_env(void *addr,int len)
{
	...
	TRACE("Gb28181 Write success ##size = %d\n", len);
	return 0;
}

static int gb28181_read_env(void *addr,int len)
{
	SYSCONF_t* sys_conf = SYSCONF_dup();
	GB_ASSERT(len < sizeof(sys_conf->ipcam.gb28181_param), "No space in sysconf for Gb28181");
	memcpy(addr, sys_conf->ipcam.gb28181_param.buf, len);

	TRACE("Gb28181 Read success ##size = %d\n", len);
	return 0;
}
...
int gb28181_init_env(pSipEnv *env)
{
	...
	*env = p;
	...
	if(strlen(gb_conf.SipServerId) > 0)
		//if(0)
	{
		//TRACE("ENV in flash,SipServerId:%s:%d\n",gb_conf.SipServerId,strlen(gb_conf.SipServerId));	
		...
		SIP_ENV_insert(p,"MaxCamera",ITOA(gb_conf.VideoNum));
		for(i=0; i<gb_conf.VideoNum; i++){
			char VideoId[32];
			sprintf(VideoId,"Video%d",i+1);
			SIP_ENV_insert(p,VideoId,gb_conf.VideoId[0]);			
		}
		//SIP_ENV_insert(p,"Video1",gb_conf.VideoId[0]);
		SIP_ENV_insert(p,"MaxAlarm",ITOA(gb_conf.VideoNum));
		for(i=0; i<gb_conf.VideoNum; i++){
			char AlarmId[32];
			sprintf(AlarmId,"Alarm%d",i+1);
			SIP_ENV_insert(p,AlarmId,gb_conf.AlarmId[0]);			
		}	
	}
		...
		SIP_ENV_insert(p,"MaxCamera",ITOA(def_gb_conf.VideoNum));
		for(i=0; i<def_gb_conf.VideoNum; i++){
			char VideoId[32];
			sprintf(VideoId,"Video%d",i+1);
			SIP_ENV_insert(p,VideoId,def_gb_conf.VideoId[0]);			
		}
		SIP_ENV_insert(p,"MaxAlarm",ITOA(def_gb_conf.VideoNum));
		for(i=0; i<def_gb_conf.VideoNum; i++){
			char AlarmId[32];
			sprintf(AlarmId,"Alarm%d",i+1);
			SIP_ENV_insert(p,AlarmId,def_gb_conf.AlarmId[0]);			
		}
	}
	return 0;
}
...
int GB28181_stop()
{
	...
	for(i=0;i<entries;i++){
		...
		if(srtp->data.flag == true){
			rtp = (Rtp_t *)srtp->data.context;
			if(rtp)	RTP_destroy(rtp);
			srtp->data.context = NULL;
		}
	}
...
int GB28181_configure(const void *xml)
{
	if(!xml)	return -1;
	char *conf_xml = (char*)xml;
	TRACE("new CONF:\n%s\n",conf_xml);
	char *env_tag = NULL;
	...
	{
		env_tag = xml_get_tagvalue(conf_xml,env_node[i+1]);
		if(env_tag){
	...
	for(i=0;i<atoi(max_camera);i++)
	{
		char VideoId[32];
		sprintf(VideoId,"Video%d",i+1);
		env_tag = xml_get_tagvalue(conf_xml,VideoId);
		if(env_tag){
	...
	for(i=0;i<atoi(max_alarm);i++)
	{
		char AlarmId[32];
		sprintf(AlarmId,"Alarm%d",i+1);
		env_tag = xml_get_tagvalue(conf_xml,AlarmId);
		if(env_tag){
