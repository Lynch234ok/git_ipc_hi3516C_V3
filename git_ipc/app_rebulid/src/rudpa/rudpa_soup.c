		...
		case SoupCmdPtz:
			{
				sprintf(buf,"<SOUP version=\"1.0\"><ptz error=\"%d\" ticket=\"%s\"/></SOUP>",
							thiz->soup_error,thiz->soup_ticket);
				pSoupPkt = strdup(buf);				
			}
			break;
...
#define SDK_ENC_BUF_DATA_G711U (0x80000002)
	...
	if( (attr->type  & SDK_ENC_BUF_DATA_PCM) == SDK_ENC_BUF_DATA_PCM)
	{
		/*audio*/
		return 0;
	}
...
char* GetStreamName(SoupData *soup_data)
{
	SoupData *thiz = soup_data;
	char *stream_buf[32];
	sprintf(stream_buf,"ch%d_%d.264",thiz->streamreq_ch,thiz->streamreq_stream_No);
	char *p = strdup(stream_buf);
...
{	
	...
	/*parse the soup data*/
	char *tmpData = (char*)calloc(nDatasize, 1);
	memcpy(tmpData, pData,nDatasize);
	ezxml_t soup_xml = ezxml_parse_str(pData ,strlen(pData));
	ezxml_t soup_cmd = soup_xml->child;
	char *tag = NULL;
	if(NULL == soup_cmd)
	{
		_RUDPA_ERROR("NULL Soup string\n");
		return -1;
	}
	...
	{		
		...
		soupData->ptz_act = ezxml_attr(soup_cmd,"act") ? strdup(ezxml_attr(soup_cmd,"act")):strdup("");
		soupData->ptz_param[0] = ezxml_attr(soup_cmd,"param1") ? strdup(ezxml_attr(soup_cmd,"param1")):strdup("");
		soupData->ptz_param[1] = ezxml_attr(soup_cmd,"param2") ? strdup(ezxml_attr(soup_cmd,"param2")):strdup("");
	...
	else if(0 == strcmp("vcon", ezxml_name(soup_cmd)))
	{
		soupData->soup_cmd = SoupCmdVcon;
		soupData->vcon_cmd = (tag = ezxml_attr(soup_cmd, "cmd")) ? strdup(tag) : strdup("");
		if( 0 == strcmp("data", soupData->vcon_cmd)){
			soupData->vcon_loadlen  = (tag = ezxml_attr(soup_cmd, "length")) ? atoi(tag) : 0;
			_RUDPA_TRACE("pData:%s\n", strstr(tmpData,"</SOUP>")+strlen("</SOUP>"));
			if(soupData->vcon_loadlen > 0){
				soupData->vcon_load = (char *)calloc(soupData->vcon_loadlen, 1);
				memcpy(soupData->vcon_load, strstr(tmpData,"</SOUP>")+strlen("</SOUP>"), soupData->vcon_loadlen);
			}
			free(tmpData);
		}
		soupData->vcon_id = (tag = ezxml_attr(soup_cmd, "id")) ? atoi(tag) : 0;
		soupData->vcon_app = (tag = ezxml_attr(soup_cmd, "app")) ? strdup(tag) : strdup("");
		soupData->vcon_ename = (tag = ezxml_attr(soup_cmd, "ename")) ? strdup(tag) : strdup("");		
	} 
	else
	{
