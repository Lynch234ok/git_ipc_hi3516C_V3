...
char  gESEE_MAC[32];
char *gESEE_PLAT_IP = "192.168.2.2";
uint32_t gESEE_PLAT_PORT = 60101;
uint32_t gRudpaLocalListenPort = 0;
...
static void *test_send_data(void *args)
{
	...
	prctl(PR_SET_NAME, "rudpa_test_send_data");
			...
			if(mediabuf_user)
			{
				...
				for(;;)
				{
					...
					if(0 == MEDIABUF_out_lock(mediabuf_user))
					{
						...
						int ret = MEDIABUF_out(mediabuf_user, (void**)&attr, NULL, &out_size);
						if(0 == ret)							
						{
						...
						{
							//printf("\033[31mMEDIABUF_out_lock failed ret:%d\n\033[0m", ret);
						}
...
void ld_packet_proc(void *data,int packsize,void *priv)
{
	...
	switch(soup_data->soup_cmd)
	{
		case SoupCmdPtz:
			{
				...
				aside_send(thiz->s,pSoupPkt, strlen(pSoupPkt));
			}
			break;
		...
		case SoupCmdReqStream:
			{		
				...
				{				      
					...
					if(0 != thiz->m_uiIp){
						soup_data->streamreq_stream_No = 1;
						thiz->m_stream  = GetStreamName(soup_data);
					}else{
						thiz->m_stream = GetStreamName(soup_data);
					}	
		...
		case SoupCmdAuth:
			{
				...
				if(soup_data->auth_usr != NULL &&
					soup_data->auth_psw != NULL &&
					USRM_check_user(soup_data->auth_usr,soup_data->auth_psw) == USRM_GREAT)
				{
					thiz->m_SoupAuthOk = !0;
		...
		case SoupCmdVcon:
			{
				_RUDPA_TRACE("vcon req?\n");
				if(0 == strcmp("create", soup_data->vcon_cmd)){
					vcon_create(soup_data->vcon_id,soup_data->vcon_app);
				}else if(0 == strcmp("data", soup_data->vcon_cmd)){
					vcon_send(thiz->s, soup_data->vcon_id,soup_data->vcon_load, soup_data->vcon_loadlen);
				}else if(0 == strcmp("destroy", soup_data->vcon_cmd)){
					vcon_destroy(soup_data->vcon_id);
				}else if(0 == strcmp("event", soup_data->vcon_cmd)){
					_RUDPA_TRACE("vcon event\n");
				}
			}
			break;
		default:
...
int RUDPA_init()
{
	...
	char sn[32] = "JA";	
	if(0 == UC_SNumberGet(&sn[2])) {
	}
	strncpy(gESEE_MAC,sn,32);
	printf("ESEE DEVICE ID:%s\r\n",gESEE_MAC);
	if(0 == gESEE_MAC[2] && 0 ==gESEE_MAC[3])
	{
