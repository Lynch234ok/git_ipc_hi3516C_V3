...
ERROR_CODE_t send_data(SESSION_HANDLE *session,char *data,u_int32_t datalen)
{
	...
	s->sendflag	= 1;//printf("session hold:%p\n",s);
	memset(s->ack,0,pktcount);
	s->pkt_id   = pktid;
	ret = session_send_data(s,pktcount,headlen);
	//printf("session_send_data:%d\n",ret);
	//send_go(s,pktcount,headlen);
	s->sendflag = 0;//printf("session release:%p\n",s);
...
RESND:
	ret = send_data(thiz->session, thiz->data, thiz->datalen);
	printf("ret:%d\n", ret);
	if((ret != 0) && (++resnd_cnt < 10))
	{
		goto RESND;
	}
	...
	if(resnd_cnt > 10){
		_RUDPA_ERROR("the shitting network!!\n");
	}
	free(thiz->data);
	free(thiz);
	thiz = NULL;
...
void aside_send(void *session,void *data, uint32_t datalen)
{
	...
	//_RUDPA_TRACE("asideData:%s\n",data);
	asideData->session= session;
	asideData->data = (char*)data;
	asideData->datalen  = datalen;
