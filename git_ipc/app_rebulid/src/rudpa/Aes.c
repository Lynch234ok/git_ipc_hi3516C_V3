...
static unsigned char State[4][4];  // State matrix
static int Nb;         // block size in 32-bit words.  Always 4 for AES.  (128 bits).
static int Nk;         // key size in 32-bit words.  4, 6, 8.  (128, 192, 256 bits).
static int Nr;         // number of rounds. 10, 12, 14.
static unsigned char key[16] = {0x55, 0x93, 0xe3, 0x7d, 0xa5, 0x64, 0x1a, 0x85, 0xb6, 0xc9, 0xac, 0x24, 0xc3, 0x2c,0xbf, 0x2c};     // the seed key. size will be 4 * keySize from ctor.
static unsigned char w[16*15];      // key schedule array. (Nb*(Nr+1))*4
...
void RandString(unsigned char* output){
	long randNum;
	unsigned char str[16];
	memset(str, 0, 16);
	srand((unsigned int)time(0));
	randNum = rand();
	sprintf((char *)str, "%ld", randNum);
	memcpy(output, str, 16);
}
