...
extern uint32_t gRudpaLocalListenPort;
...
int sock_create(char *ip,u_int16_t port)
{
	...
	getsockname(sock_fd, &servaddr,&optlen);
	gRudpaLocalListenPort = ntohs(servaddr.sin_port);	
	rudpa_log("/tmp/rudpa.port", "w+", "%d",gRudpaLocalListenPort);	
...
static void *session_listen(void *proc)
{
	...
	struct sockaddr_in 	clientaddr;
	prctl(PR_SET_NAME, "session_listen");
	for(;;)
	{
		tv.tv_sec =2;
		tv.tv_usec=0;
	...
	linster_destroy(l);
	_linster_unlock(l);	
	_RUDPA_ERROR("rudpa listen(recv) thread abort!!\n");
