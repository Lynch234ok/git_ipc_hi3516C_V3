	...
	switch(dwCommand){
		...
		case ANTS_MID_SET_NETCFG:
		{
			...
			if(lpNetCfg->dwSize == sizeof(ANTS_MID_NETCFG)){
				...
				lan.lan.addressingType = lpNetCfg->byUseDhcp ? 1 : 0;// 是否启用DHCP 0xff-无效 0-不启用 1-启用
				http_port.value = lpNetCfg->wHttpPortNo;/* HTTP端口号 */
	...
	if(lpBuffer==NULL||lpOutSize==NULL||dwInSize<=0)
		...
		case ANTS_MID_GET_NETCFG:
		{
			...
			if(0 == ifconf_get_interface(_ants_server.server_eth, &ifr)){
				...
				// ifconf暂缺这个接口，需补充
				strcpy(lpNetCfg->struDnsServer1IpAddr.sIpV4, lan.dns.staticPreferredDns);// 域名服务器1的IP地址
				strcpy(lpNetCfg->struDnsServer2IpAddr.sIpV4, lan.dns.staticAlternateDns);// 域名服务器2的IP地址

				lpNetCfg->wAlarmHostIpPort = 0;// 报警主机端口号
				lpNetCfg->byUseDhcp = lan.lan.addressingType;// 是否启用DHCP 0xff-无效 0-不启用 1-启用
				...
				lpNetCfg->wHttpPortNo = port.value;/* HTTP端口号 */
