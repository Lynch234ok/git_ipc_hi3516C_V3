...
typedef struct NTP
{
	...
	char server_backup_one[32];
	char server_backup_two[32];
	char *server_ip;
	int timeout;
	//char timezone;
	int minutesWest;
...
#define NTP_MAXSTRATUM	(16)
	...
	if(wait_sec > 0){
		...
		if((LI_ALARM & ret_pack->li_vn_mode) == LI_ALARM
			|| ret_pack->stratum == 0
			|| ret_pack->stratum > NTP_MAXSTRATUM
			|| ret_pack->recvtimestamphigh == 0
			|| ret_pack->trantimestamphigh == 0){
			printf("%s peer is unsynced, li_vn_mode %x, stratum %d, recvtimestamphigh %d, trantimestamphigh %d\n", 
				__func__, ret_pack->li_vn_mode, ret_pack->stratum, ret_pack->recvtimestamphigh, ret_pack->trantimestamphigh);
			return -1;
		}
		return 0;
	}
...
//	while(i<_ntp->timeout)
	while(1)
	{
		sock = ntp_sock_create();
		if(sock < 0)
		{
			APP_TRACE("ntp create socket failed! try again...");
			usleep(1000000);
			continue;
		}
		...
		if(sync_times >= 0 && sync_times <= 2 && ((_ntp->server_domain && strlen(_ntp->server_domain) != 0) || _ntp->server_ip)){
			ntp_sock_send(sock, _ntp->server_domain, _ntp->server_ip, NTP_SERVER_PORT, send_pts);
		}else if(sync_times >= 3 && sync_times <= 5 && ((_ntp->server_backup_one && strlen(_ntp->server_backup_one) != 0) || _ntp->server_ip)){
			ntp_sock_send(sock, _ntp->server_backup_one, _ntp->server_ip, NTP_SERVER_PORT, send_pts);
		}else if(sync_times >= 6 && sync_times <= 8 && ((_ntp->server_backup_two && strlen(_ntp->server_backup_two) != 0) || _ntp->server_ip)){
			ntp_sock_send(sock, _ntp->server_backup_two, _ntp->server_ip, NTP_SERVER_PORT, send_pts);
		}else{
		...
		// wait response
		ret = ntp_sock_recv(sock, 6, &ntp_pack);
		if(0 == ret){
			break;
		}else{
...
// 因为最新的修改，ntp同步时不设置时区，所以参数4不生效
int NTP_start(const char* server_domain, const char* server_backup_one, const char* server_backup_two, const char* server_ip, int timeout, int minutesWest)
{
	if(!_ntp){
		_ntp = calloc(sizeof(NTP_t), 1);
		//assert(_ntp);
        if (NULL != server_domain) {
            snprintf(_ntp->server_domain, sizeof(_ntp->server_domain), "%s", server_domain);
        } else {
            _ntp->server_domain[0] = '\0';
        }
        if (NULL != server_backup_one) {
            snprintf(_ntp->server_backup_one, sizeof(_ntp->server_backup_one), "%s", server_backup_one);
        } else {
            _ntp->server_backup_one[0] = '\0';
        }
        if (NULL != server_backup_two) {
            snprintf(_ntp->server_backup_two, sizeof(_ntp->server_backup_two), "%s", server_backup_two);
        } else {
            _ntp->server_backup_two[0] = '\0';
        }
		_ntp->server_ip = server_ip;
		_ntp->timeout = timeout;
		_ntp->minutesWest = minutesWest;
		_ntp->ntp_trigger = 0;
		_ntp->ntp_tid = (pthread_t)NULL;
		
		int ret = 0;
		_ntp->ntp_trigger = 1;
		ret = JA_THREAD_init0(&_ntp->ntp_tid, (THREAD_PROC)NTP_sync, NULL, NULL, 0, NULL, 131072, 0);
		//assert(0 == ret);
	}
