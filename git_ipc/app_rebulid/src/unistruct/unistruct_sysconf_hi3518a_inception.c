...
static int unistruct_hi3518a_inception_rw(SYSCONF_t* const sysconf, const char* xpath, char* io_text, SYS_BOOL_t opt_rw)
{
	char attr[128];
	int ain_ch = 0;
	int ain_encode_ch = 0;
...
#define UNI_ATTR_MATCH(a_name) (0 == strcmp(attr, a_name))
	...
	// vin
		...
		if(UNI_ATTR_MATCH("shutter")){
			...
			unistruct.do_check = do_check_hue;
			unistruct.do_policy = do_policy_hue;
