...
{
//	APP_TRACE("_ctx->tmp.auth.user=%s, _ctx->tmp.auth.password=%s", _ctx->tmp.auth.user, _ctx->tmp.auth.password);
	if(_ctx->tmp.conf.user != NULL && _ctx->tmp.conf.password != NULL)
	{
		if(USRM_GREAT == USRM_check_user(_ctx->tmp.conf.user, _ctx->tmp.conf.password)){
	...
	APP_TRACE("_ctx->auth_status=%d", _ctx->auth_status);
	APP_TRACE("gw_cgi login:%s-%s\n", _ctx->tmp.conf.user, _ctx->tmp.conf.password);
...
{
//	APP_TRACE("_action_attr_in->path=%s", _action_attr_in->path);
	char value[128] = {0};
	...
	else if(strcmp(_action_attr_in->path , "/juan/setup/@password") == 0)
	{
		if(_ctx->tmp.conf.password == NULL)
		{
			...
			_check_auth(_ctx);
			usrm = USRM_login(_ctx->tmp.conf.user, _ctx->tmp.conf.password);
			if(usrm != NULL){
				_ctx->permit = usrm->permit_flag;
				USRM_logout(usrm);
			}
		}
	}
	else
	{
		if(_ctx->auth_status == GW_CGI_CTX_AUTH_STATUS_SUCCESS)
		{
			...
			{
				if((_ctx->permit & USRM_PERMIT_SETTING) != 0){
					char* left = _key;
...
//	APP_TRACE("_action_attr_in->path=%s", _action_attr_in->path);
	...
	else if(strcmp(_action_attr_in->path , "/juan/conf/@password") == 0)
	{
		if(_ctx->tmp.conf.password == NULL)
		{
			...
			if(usrm != NULL){
				_ctx->permit = usrm->permit_flag;
				APP_TRACE("gw_cgi get permit:%s-%s\n", _ctx->tmp.conf.user, _ctx->tmp.conf.password);
				USRM_logout(usrm);
			}
		}
	}
...
//	APP_TRACE("_action_node_in->path=%s, attr_count=%d", _action_node_in->path, attr_count);
	...
	if(attr_count > 0)
	{
		for(i = 0; i < attr_count; i++)
		{
			...
			//APP_ASSERT(attr_in != NULL, "_action_node_in->path=%s, i=%d", _action_node_in->path, i);

//			APP_TRACE("attr_in->path=%s, attr_in->name=%s", attr_in->path, attr_in->name);
...
//	APP_TRACE("children_count=%d", children_count);
	...
	if(children_count > 0)
	{
		for(i = 0; i < children_count; i++)
		{
			...
			//APP_ASSERT(node_in != NULL, "_action_node_in->path=%s, i=%d", _action_node_in->path, i);
...
//	APP_TRACE("return_ret=%d", return_ret);
	return return_ret;
}

int cgi_gw_main(HTTPD_SESSION_t* _session)
{
	APP_TRACE("%s start", __FUNCTION__);
	int return_ret = 0;
...
#define MAX_XML_LENGTH (1024*10)
	...
	APP_TRACE("nParam_count=%d", param_count);
...
//		APP_TRACE("gw_ctx.auth_status=%d", gw_ctx.auth_status);
...
exit_cgi_gw_main2:
	APP_TRACE("%s end, ret=%d", __FUNCTION__, return_ret);
	return return_ret;
}

int cgi_gw_main2(LP_HTTP_CONTEXT context)
{
	int return_ret = 0;
...
#define MAX_XML_LENGTH (1024*10)
	...
	char raw_xml[MAX_XML_LENGTH] = {0};
	char xml[MAX_XML_LENGTH] = {0};
	unsigned char write_flag = false;

	_URL_PARAM_t params[MAX_PARAM_IN_URL];
	memset(&params, 0, sizeof(_URL_PARAM_t)*MAX_PARAM_IN_URL);
	AVal query_string; // = AVC(context->request_header->query);
	query_string.av_val = context->request_header->query;
	query_string.av_len = strlen(context->request_header->query);
	int param_count = CGI_parse_url(query_string, params);
...
//		int nRet = -1;
		if (0 < strlen(xml)) {
			...
			//SOCKET_RW_CTX rw_ctx;
			//SOCKETRW_rwinit(&rw_ctx, context->sock, (void *)ack_buf, ack_size, context->keep_alive);
			//SOCKETRW_writen(&rw_ctx);
			//if(rw_ctx.result != SOCKET_RW_RESULT_SUCCESS)
			//{
			//	printf("send error\r\n");
			//}
			send(context->sock, ack_buf, ack_size, 0);
			//APP_TRACE("%s", ack_buf);
		}
