...
size_t ts_demuxer_input(struct ts_demuxer_t* ts, const uint8_t* data, size_t bytes)
{
	...
	if(0x47 != data[0])
	{
		printf("[%s:%d]TS stream sync byte error(0x47 != data[0])\n",__func__,__LINE__);
		return 1;
	}
...
//		printf("[%s:%d]adaptation_field_length:%d,PCR_flag:%d\n",__func__,__LINE__,pkhd.adaptation.adaptation_field_length,pkhd.adaptation.PCR_flag);
