...
typedef int (*fRTSP_GET_PWD_BY_NAME)(const char *name, char *ret_pwd, int ret_size);
extern void HTTP_AUTH_set_rtsp_auth_hook(fRTSP_GET_PWD_BY_NAME hook);
...
extern int HTTP_AUTH_validate(struct _authentication *auth,
	...
	char *method);
extern int HTTP_AUTH_validate2(struct _authentication *auth, char *authorization);
extern int HTTP_AUTH_get_auth(struct _authentication *auth, char *authorization);
