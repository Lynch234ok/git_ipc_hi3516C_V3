...
#enable sdcard power and load himciv200.ko
/media/custom/ipcam/shell/sdcard_power.sh
...
if [ "$char" == "f" ]; then
	...
	if [ "0" == "$?" ]; then
		...
		do
			...
			LOG_FILE=ipc_$CURTIME$RANDOM.log
			echo $LOG_FILE
			$APPEXEC -w $WEBDIR -f $FONTDIR -m $FLASHMAP -s $SYSCONF -d $NETSDKDIR_DEF -c $NETSDKDIR 2>&1 1>> "$MOUNT_DIR/$LOG_FILE"
		done
	fi
