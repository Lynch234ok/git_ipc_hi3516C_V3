...
function isp_save_content()
{
	...
	var scene_mode,white_balance_mode,ircut_control_mode,ircut_mode,wide_dynamic_range_enable,exposure_mode,denoise_enable,anti_fog_enable,lowlight_enable,gamma,defect_pixel_enable,image_style;
	...
	var isp_data = '{"sceneMode": "'+ scene_mode +'","imageStyle":"'+image_style+'","awbMode":"'+white_balance_mode+'","lowlightMode":"'+lowlight_enable+'","irCutFilter" : {"irCutControlMode" : "' + ircut_control_mode +'","irCutMode" : "' + ircut_mode +'",},"denoise3d" : {"denoise3dStrength" : ' + denoise3dStrength +',},"WDR" : {"enabled" : ' + wide_dynamic_range_enable +',"WDRStrength" : ' + wdr_str +',},}'
...
function videoMode_save(fixMode, showMode){
	var fixMode_str;
	var showMode_str;
	var content;
	var cellMode=['360','4R'];
	//var cellMode=['origin','360','4R','split','wall_split','180', 'kitR', 'kitO'];
	var wallMode=['origin','4R','split','wall_split','180'];
	var tableMode=['origin','360','4R','split','VR', 'kitR', 'kitO'];
	switch(fixMode){
		case 0:{//cell
			content =  '{"fixMode": "cell","cellMode":"'+cellMode[showMode] +'"}';
			break;
		}	
		case 1:{//wall
			content =  '{"fixMode": "wall","wallMode":"'+wallMode[showMode] +'"}';
			break;	
		}
		case 2:{//table
			content =  '{"fixMode": "table","tableMode":"'+tableMode[showMode] +'"}';
			break;
		}
	}
	var auth = "Basic " + base64.encode(g_usr+':'+g_pwd);
	$.ajax({
			type:'PUT',
			url:dvr_url + '/netsdk/image/videoMode',
			dataType:'json',
			data:content,
			async:false,
			beforeSend : function(req ) {
...
function videoMode2ui(fixMode, showMode){
	var fixMode_focus=['cell_focus','wall_focus','table_focus'];
	var fixMode_p=['cell','wall','table'];
	
	//cell mode
	/*var cell_focus=['origin_focus','360_focus','4Rr_focus','split_focus','wall_split_focus','180_focus', 'kitR_focus', 'kitO_focus'];
	var cell_p=['origin','360','4R','split','wall_split','180', 'kitR', 'kitO'];*/
	var cell_focus=['360_focus','4Rr_focus'];
	var cell_p=['360','4R'];
	
	//wall mode
	var wall_focus=['origin_focus','4R_focus','split_focus','wall_split_focus','180_focus'];
	var wall_p=['origin','4R','split','wall_split','180'];
	
	//table mode
	var table_focus=['origin_focus','360_focus','4R_focus','split_focus','VR_focus', 'kitR_focus', 'kitO_focus'];
	var table_p=['origin','360','4R','split','VR', 'kitR', 'kitO'];

	for(var i=0;i<fixMode_focus.length;i++){
	   if(i==fixMode){
	      $('#fixMode .wui-icon span').eq(i).css('background-image',"url(images/"+fixMode_focus[i]+".png)");
	   }else{
	      $('#fixMode .wui-icon span').eq(i).css('background-image',"url(images/"+fixMode_p[i]+".png)");
	   }
	 }
	switch(fixMode){
		case 0:{//cell
			for(var c=0;c<cell_focus.length;c++){
			   if(c==showMode){
			     $('#cellMode .wui-icon span').eq(c).css('background-image',"url(images/"+cell_focus[c]+".png)");
			   }else{
			     $('#cellMode .wui-icon span').eq(c).css('background-image',"url(images/"+cell_p[c]+".png)");
			   }
			 }
			break;
		}
		case 1:{//wall
			for(var c=0;c<wall_focus.length;c++){
			   if(c==showMode){
			     $('#wallMode .wui-icon span').eq(c).css('background-image',"url(images/"+wall_focus[c]+".png)");
			   }else{
			     $('#wallMode .wui-icon span').eq(c).css('background-image',"url(images/"+wall_p[c]+".png)");
			   }
			 }
			break;
		}
		case 2:{//table
			 for(var c=0;c<table_focus.length;c++){			 
			   if(c==showMode){
			     $('#tableMode .wui-icon span').eq(c).css('background-image',"url(images/"+table_focus[c]+".png)");
			   }else{
			     $('#tableMode .wui-icon span').eq(c).css('background-image',"url(images/"+table_p[c]+".png)");
			   }
			 }
			break;
		}
...
function videoMode_load(){
	var auth = "Basic " + base64.encode(g_usr+':'+g_pwd);
	var fixMode_focus=['cell_focus','wall_focus','table_focus'];
	var fixMode_p=['cell','wall','table'];
	
	//cell mode
	/*var cell_focus=['origin_focus','360_focus','4R_focus','split_focus','wall_split_focus','180_focus', 'kitR_focus', 'kitO_focus'];
	var cell_p=['origin','360','4R','split','wall_split','180', 'kitR', 'kitO'];*/
	var cell_focus=['360_focus','4R_focus'];
	var cell_p=['360','4R'];
		...
		$.ajax({
				...
				success:function(data){	
					...
					switch(data.cellMode){
						default:
						case "origin" : cellMode_index = 0;break;
						case "360" : cellMode_index = 1;break;
						case "4r" : cellMode_index = 2;break;
						case "split" : cellMode_index = 3;break;
						case "wall_split" : cellMode_index = 4;break;
						case "180" : cellMode_index = 5;break;
					}
					switch(data.wallMode){
						default:
						case "origin" : wallMode_index = 0;break;
						case "4r" : wallMode_index = 1;break;
						case "split" : wallMode_index = 2;break;
						case "wall_split" : wallMode_index = 3;break;
						case "180" : wallMode_index = 4;break;					
					}
					switch(data.tableMode){
						default:
						case "origin" : tableMode_index = 0;break;
						case "360" : tableMode_index = 1;break;
						case "4r" : tableMode_index = 2;break;
						case "split" : tableMode_index = 3;break;
					}
					...
					switch (data.fixMode)
					{
						case "wall": 
						{
							fixMode_index = 1;	
							/*$(".content").hide();
							$(".content").eq(fixMode_index).fadeIn();				
							for(var i=0;i<fixMode_focus.length;i++){
							   if(i==fixMode_index){
							      $('#fixMode .wui-icon span').eq(i).css('background-image',"url(images/"+fixMode_focus[i]+".png)");
							   }else{
						...
						case "cell": 
						{
							fixMode_index = 0;
							/*$(".content").hide();
							$(".content").eq(fixMode_index).fadeIn(100);
							for(var i=0;i<fixMode_focus.length;i++){
							   if(i==fixMode_index){
							      $('#fixMode .wui-icon span').eq(i).css('background-image',"url(images/"+fixMode_focus[i]+".png)");
							   }else{
						...
						case "table": 
						{
							...
							/*for(var i=0;i<fixMode_focus.length;i++){
							 ...
							 //alert(tableMode_index);
							 for(var c=0;c<table_focus.length;c++){			 
							   if(c==tableMode_index){
							     $('#tableMode .wui-icon span').eq(c).css('background-image',"url(images/"+table_focus[c]+".png)");
							   }else{
							     $('#tableMode .wui-icon span').eq(c).css('background-image',"url(images/"+table_p[c]+".png)");
							   }
					...
					$(".content").hide();
					$(".content").eq(fixMode_index).fadeIn(200, videoMode2ui(fixMode_index, tableMode_index));
				},
				error:function(a,b,c){ 
			}
		});	
