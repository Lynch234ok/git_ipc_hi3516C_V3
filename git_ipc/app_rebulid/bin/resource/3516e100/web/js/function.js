...
function isp_save_content()
{
	...
	var scene_mode,white_balance_mode,ircut_control_mode,ircut_mode,wide_dynamic_range_enable,exposure_mode,denoise_enable,anti_fog_enable,lowlight_enable,gamma,defect_pixel_enable,image_style,colortoblack_range;
	...
	switch($("#isp\\@ircut_mode")[0].selectedIndex)
	{
        ...
        case 1: ircut_mode = "light";break;
        case 2: ircut_mode = "smart";break;
        case 3: ircut_mode = "daylight";break;
        case 4: ircut_mode = "night";break;
		default:break;
	}
	...
	var isp_data = '{"sceneMode": "'+ scene_mode +'","imageStyle":"'+image_style+'","awbMode":"'+white_balance_mode+'","lowlightMode":"'+lowlight_enable+'","irCutFilter" : {"colortoblackrange" : ' + colortoblackrange +',"irCutControlMode" : "' + ircut_control_mode +'","irCutMode" : "' + ircut_mode +'",},"denoise3d" : {"denoise3dStrength" : ' + denoise3dStrength +',},"WDR" : {"enabled" : ' + wide_dynamic_range_enable +',"WDRStrength" : ' + wdr_str +',},}'
...
window.onload=function()
{
	...
	$('#ptz-controller-cam').click(function(){
			...
			preset_common(ptz_act_auto_status,ptz_act,ptz_speed);
		});
...
function motion_dection_data2ui()
{
	...
	var X,Y,disX,disY;
	var motionBack = $('<div class="motion_detection_bg" style=" width: 640px; border:1px solid red; position: relative; z-index: 1; background-image:url(http://'+tmp_ip+'/snapshot?r='+ snap +'&auth='+auth+');"></div>').appendTo($('#motion_detection_background'));	
	...
	for(var i=0;i<18;i++)
		for(var j=0;j<22;j++)
			{
	      		var oMove_view = $('<div id="oView'+k+'" style=" display:block; height:20px; width:29px;   border: 1px solid #CCBBFF; position:absolute; left:'+j*29+'px; top:'+i*20+'px"></div>').appendTo($('div.motion_detection_bg'));
				arr1[i][j] = false;
				k++;
			}
	...
	$(document).mousemove(function(e){
			...
			for(var i=0;i<18;i++)
			{
			    ...
			    for(var j=0;j<22;j++)
			    	{
						t=i*22+j;
						if(i>=Math.floor(T/20)&&i<=Math.floor(T/20)+Math.ceil(H/20)&&j>=Math.floor(L/29)&&j<=Math.floor(L/29)+Math.ceil(W/29))
						{
							arr1[i][j]=true;
...
{
	showInfo(langstr.save_setup);
	var motion_data,motion_data1,motion_data2,motion_data3,motion_data4,motion_data5,motion_enabled,columnGranularity=22,rowGranularity=18,m,n;
	var arr2 = arr1.join(',');
...
function overlay_data2ui()
{
	...
	$('#Overlay_background').find('div').remove();	
		var overlayBack = $('<div class="overlay_bg" style=" width: 640px; border:1px solid red; position: relative; z-index: 1; background-image:url(http://'+tmp_ip+'/snapshot?r='+ snap +'&auth='+auth+');"></div>').appendTo($('#Overlay_background'));	
...
{	
		...
		$('#Cover_background').find('div').remove();	
			var coverBack = $('<div class="cover_bg" style=" width: 640px; border:1px solid red; position: relative; z-index: 1; background-image:url(http://'+tmp_ip+'/snapshot?r='+ snap +'&auth='+auth+');"></div>').appendTo($('#Cover_background'));	
